import{a as N,R as H,L as B,b as ve,H as ge,V as v,c as A,S as de,O as ue,d as fe,e as M,M as me}from"./three-vlqji54k.js";import{m as we}from"./random-DL1jLgMw.js";const xe=512,ke=256,V=2,be=1,U=6,ye=4,R=4,Se=240,Ae=60,C=1/30,z=256,Me=.05,E=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`}const p=[0,56,128,172,232,316,336.998],f=[{name:"thriving-field",vitalityTarget:.95,churn:.9,presence:.8,lifeRate:.1,wriggle:1,drift:1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.45,fxAmount:.7,aura:0,beatPop:0,eventVivid:0,gridStrength:0,gridFine:0,glyphDensity:.85,chatterRate:1.2,machineFrac:0,classifiedFloor:0,commFreq:6,zoom:1.18,survivorFocus:0,hueSat:1,warmth:.55,rustMix:.05,inkPersist:0,vignette:.3,motes:.15,groundLight:.9},{name:"first-scans",vitalityTarget:.85,churn:.8,presence:.85,lifeRate:.12,wriggle:.8,drift:.8,scanRate:5,scanDrain:.5,misProb:.3,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.7,fxAmount:.8,aura:0,beatPop:.25,eventVivid:.3,gridStrength:.18,gridFine:.25,glyphDensity:.8,chatterRate:1,machineFrac:.08,classifiedFloor:.06,commFreq:6,zoom:1.05,survivorFocus:0,hueSat:.92,warmth:.5,rustMix:.15,inkPersist:.5,vignette:.32,motes:.08,groundLight:.9},{name:"accelerating-catalogue",vitalityTarget:.7,churn:.75,presence:.8,lifeRate:.16,wriggle:.7,drift:.6,scanRate:16,scanDrain:.7,misProb:.15,classifyPressure:0,waveRate:0,linkRate:2,runnerRate:4,gridLife:.7,fxAmount:.6,aura:0,beatPop:.4,eventVivid:.5,gridStrength:.42,gridFine:.5,glyphDensity:.7,chatterRate:.9,machineFrac:.3,classifiedFloor:.28,commFreq:6,zoom:.98,survivorFocus:0,hueSat:.8,warmth:.45,rustMix:.35,inkPersist:.8,vignette:.35,motes:.05,groundLight:.9},{name:"last-unclassified",vitalityTarget:.9,churn:.6,presence:.75,lifeRate:.08,wriggle:1,drift:.7,scanRate:1.5,scanDrain:.4,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.3,fxAmount:.5,aura:1,beatPop:.3,eventVivid:.4,gridStrength:.1,gridFine:.15,glyphDensity:.9,chatterRate:.7,machineFrac:.35,classifiedFloor:.34,commFreq:6,zoom:2.9,survivorFocus:1,hueSat:.85,warmth:.5,rustMix:.3,inkPersist:.5,vignette:.5,motes:.05,groundLight:.9},{name:"total-classification",vitalityTarget:.25,churn:.5,presence:.9,lifeRate:.3,wriggle:.35,drift:.25,scanRate:34,scanDrain:1,misProb:0,classifyPressure:.35,waveRate:10,linkRate:14,runnerRate:22,gridLife:1,fxAmount:.35,aura:0,beatPop:1,eventVivid:1,gridStrength:1,gridFine:1,glyphDensity:.55,chatterRate:1.4,machineFrac:.9,classifiedFloor:.9,commFreq:6,zoom:.82,survivorFocus:0,hueSat:.4,warmth:.35,rustMix:.55,inkPersist:1,vignette:.42,motes:.02,groundLight:.9},{name:"residue",vitalityTarget:.1,churn:.25,presence:.22,lifeRate:.04,wriggle:.15,drift:.1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.3,fxAmount:.5,aura:0,beatPop:.15,eventVivid:.2,gridStrength:.06,gridFine:0,glyphDensity:.08,chatterRate:.3,machineFrac:1,classifiedFloor:.93,commFreq:6,zoom:1.15,survivorFocus:0,hueSat:.2,warmth:.3,rustMix:.45,inkPersist:.35,vignette:.45,motes:.5,groundLight:1}],k=[[0,.22],[30,.34],[36,.28],[54,.3],[56,.36],[100,.45],[126,.5],[128,.58],[167.8,.68],[168.3,.78],[171,.74],[174,.42],[200,.38],[228,.5],[231.7,.55],[232.2,.96],[246,1],[300,.82],[315.6,.7],[316.4,.18],[330,.08],[336.998,.04]],W={energy:0};function De(c){const e=Math.min(Math.max(c,0),k[k.length-1][0]);let a=0;for(;a<k.length-2&&e>=k[a+1][0];)a++;const t=k[a],i=k[a+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,i[0]-t[0])));return W.energy=t[1]+(i[1]-t[1])*s,W}const b=[[0,0],[128,0],[172,.22],[232,.25],[300,.95],[316,1],[336.998,1]],K={order:0};function Ie(c){const e=Math.min(Math.max(c,0),b[b.length-1][0]);let a=0;for(;a<b.length-2&&e>=b[a+1][0];)a++;const t=b[a],i=b[a+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,i[0]-t[0])));return K.order=t[1]+(i[1]-t[1])*s,K}const Le=6;function Oe(c){const e=Math.min(1,Math.max(0,c));return e*e*(3-2*e)}function _e(c,e,a){if(a<=0)return c;if(a>=1)return e;const t={...c,name:a<.5?c.name:e.name};for(const i of Object.keys(c)){const s=c[i],o=e[i];typeof s=="number"&&typeof o=="number"&&(t[i]=s+(o-s)*a)}return t}function Ge(c){const e=p[p.length-1],a=Math.min(Math.max(c,0),e-.001);let t=0;for(;t<f.length-1&&a>=p[t+1];)t++;const i=p[t],s=p[t+1]??e,o=Math.min(1,Math.max(0,(a-i)/Math.max(.001,s-i))),r=t<f.length-1,h=s-a,u=r?Oe(1-Math.min(1,h/Le)):0,d=f[t],w=r?f[t+1]:d;return{params:_e(d,w,u),actIndex:t,localT:o,blend:u}}const T=90;function F(c,e){const a=Math.max(1e-9,e)/60,t=Math.max(1e-6,c());return Math.min(T,-Math.log(t)/a)}const Ne=.25,He=.08,Be=.06,P=.5,Ve=.25,Ue=.06,ze=.05,q=.22,D=.35,We=.18,I=1.2,Ke=1.6,qe=.75,Xe=1.2,Ye=.5,X=4,je=1.2,$e=2,Ze=1,Je=4,Qe=2,et=.5,L=1.4,tt=2.2,it=.42,at=.12,st=.012,ot=1.2,O=1.6,lt=.8,nt=1,rt=3,Y=.35,ct=1.6,ht=1.6,dt=4,ut=8,ft=2,mt=4,pt=.5,vt=.7,j=168,$=232,Z=316,J=330,gt=.6,wt=1,xt=1.5,kt=6,bt=.5,Q=4,ee=16,te=8,yt=6,ie=14,ae=8,St=5,At=3,se=3,oe=9.5,Mt=40,Tt=8,le=.45,Rt=.015,Ct=.22;function Et(c,e){const a=c*127.1+e*311.7,t=c*269.5+e*183.3,i=Math.sin(a)*43758.5453,s=Math.sin(t)*43758.5453;return[i-Math.floor(i),s-Math.floor(s)]}const[Ft,Pt]=Et(0,0),ne={x:.22+.56*Ft,y:.22+.56*Pt},re=6,ce={x:ne.x/re,y:ne.y/re},he=5e-4,Dt=10,S=1.5,_=.6,It=.35,Lt=.05,Ot=1;class _t{renderer;scene;camera;field;quad;material;rand;forceScanAlways=!1;forceMisAlways=!1;forceWaveAlways=!1;forceFlickerAlways=!1;forceSparkAlways=!1;forceLinkAlways=!1;forceRunAlways=!1;forceLifeFast=!1;forceKillAlways=!1;forceAuraAlways=!1;forcePopAlways=!1;pinnedClassified=null;pinnedOrder=null;pinnedGridMode=null;gridMode=0;gridModeTimer=0;gridModePending=!1;gridModePendingElapsed=0;fxMode=0;fxActive=!1;fxAge=0;fxDuration=0;fxTimeToNext=0;pinnedFx=null;full=!0;stampSlotCount=U;stepsPerFrame=V;cover=new A(1,1);pan=new A(0,0);userPanned=!1;bassE=0;midE=0;highE=0;bassSlowE=0;highSlowE=0;onsetCooldown=0;highOnsetCooldown=0;scanOnsetCooldown=0;linkOnsetCooldown=0;runnerOnsetCooldown=0;flash=0;beatPulse=0;lifeClock=0;beatCount=0;beatFlash=0;spark=0;sparkSeed=0;sparkDebugTimer=0;scanSlots=[];scanA=[];scanB=[];scanTimeToNext=0;scanScheduleRate=0;scanDebugTimer=0;pendingBurst=0;burstTimer=0;waveActive=!1;waveAge=0;waveCx=.5;waveCy=.5;waveTimeToNext=0;waveDebugTimer=0;rippleSlots=[];rippleValues=[];pokeSlots=[];linkSlots=[];linkA=[];linkB=[];linkTimeToNext=0;linkScheduleRate=0;linkDebugTimer=0;onsetCount=0;runnerSlots=[];runnerValues=[];runnerTimeToNext=0;runnerScheduleRate=0;runDebugTimer=0;killSlots=[];killValues=[];killDebugTimer=0;popDebugTimer=0;classified=0;gridSlam=0;flickerEnv=0;breathPhase=0;firstUpdate=!0;lastDt=0;lastSongTime=-1;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;init(e){const{renderer:a,seed:t,quality:i}=e;this.renderer=a,this.rand=we(t^2999220761);const s=new URLSearchParams(location.search),o=s.get("solo"),r=o==="field"?1:o==="patch"?2:o==="glyphs"?3:o==="machine"?4:0;this.forceScanAlways=s.get("scan")==="always",this.forceMisAlways=s.get("mis")==="always",this.forceWaveAlways=s.get("wave")==="always",this.forceFlickerAlways=s.get("flicker")==="always",this.forceSparkAlways=s.get("spark")==="always",this.forceLinkAlways=s.get("link")==="always",this.forceRunAlways=s.get("run")==="always",this.forceLifeFast=s.get("life")==="fast",this.forceKillAlways=s.get("kill")==="always",this.forcePopAlways=s.get("pop")==="always";const h=s.get("classified");if(h!==null){const l=parseFloat(h);Number.isNaN(l)||(this.pinnedClassified=Math.min(1,Math.max(0,l)))}const u=s.get("order");if(u!==null){const l=parseFloat(u);Number.isNaN(l)||(this.pinnedOrder=Math.min(1,Math.max(0,l)))}const d=s.get("gridmode");if(d!==null){const l=parseInt(d,10);!Number.isNaN(l)&&l>=0&&l<Q&&(this.pinnedGridMode=l)}this.forceAuraAlways=s.get("aura")==="always";const w=s.get("fx");if(w!==null){const l=parseInt(w,10);!Number.isNaN(l)&&l>=0&&l<se&&(this.pinnedFx=l)}this.full=i.level==="full",this.stampSlotCount=this.full?U:ye;const n=this.full?$e:Ze,y=this.full?Je:Qe;this.stepsPerFrame=this.full?V:be;for(let l=0;l<this.stampSlotCount;l++)this.scanSlots.push({age:0,active:!1,mislabel:!1}),this.scanA.push(new v(0,0,0,0)),this.scanB.push(new v(0,0,0,0));for(let l=0;l<X;l++)this.rippleSlots.push({age:0,active:!1}),this.rippleValues.push(new v(0,0,0,0));for(let l=0;l<R;l++)this.pokeSlots.push({age:0,active:!1});for(let l=0;l<rt;l++)this.linkSlots.push({age:0,active:!1,struck:!1}),this.linkA.push(new v(0,0,0,0)),this.linkB.push(new v(0,0,0,0));for(let l=0;l<mt;l++)this.runnerSlots.push({age:0,active:!1}),this.runnerValues.push(new v(0,0,0,0));for(let l=0;l<dt;l++)this.killSlots.push({age:0,active:!1}),this.killValues.push(new v(0,0,0,0));this.breathPhase=this.rand()*Math.PI*2,this.pinnedGridMode!==null&&(this.gridMode=this.pinnedGridMode),this.gridModeTimer=ee+this.rand()*te,this.fxTimeToNext=ae+this.rand()*ie,this.scene=new de,this.camera=new ue(-1,1,1,-1,0,1),this.field=new Ee(a,this.full,this.stampSlotCount),this.material=new M({vertexShader:Fe,fragmentShader:Pe(X,this.stampSlotCount,n,y),depthTest:!1,depthWrite:!1,uniforms:{uField:{value:null},uTime:{value:0},uCover:{value:new A(1,1)},uZoom:{value:1},uPan:{value:this.pan},uCommFreq:{value:6},uEnergy:{value:0},uFlash:{value:0},uSparkle:{value:0},uGlyphKick:{value:0},uGlyphSeed:{value:0},uChurn:{value:0},uChatter:{value:0},uGlyphDensity:{value:0},uMachineFrac:{value:0},uMachineOrder:{value:0},uClassified:{value:0},uGridStrength:{value:0},uGridFine:{value:0},uGridSlam:{value:0},uHueSat:{value:0},uWarmth:{value:0},uRustMix:{value:0},uInkPersist:{value:0},uVignette:{value:0},uMotes:{value:0},uGroundLight:{value:0},uSurvivorFocus:{value:0},uDesat:{value:0},uSoloMode:{value:r},uFlicker:{value:0},uLifeClock:{value:0},uPresence:{value:0},uWriggle:{value:0},uDrift:{value:0},uWaveVis:{value:this.field.wave},uScanA:{value:this.scanA},uScanB:{value:this.scanB},uRipple:{value:this.rippleValues},uEventVivid:{value:0},uLinkA:{value:this.linkA},uLinkB:{value:this.linkB},uRunner:{value:this.runnerValues},uKill:{value:this.killValues},uBeatCount:{value:0},uBeatFlash:{value:0},uBeatPop:{value:0},uGridLife:{value:0},uGridMode:{value:0},uAura:{value:0},uAuraPulse:{value:0},uFxMode:{value:0},uFxAmt:{value:0}}}),this.quad=new me(new fe(2,2),this.material),this.scene.add(this.quad);const x=a.domElement;this.resize(x.clientWidth||1,x.clientHeight||1)}sst(e){const a=Math.min(1,Math.max(0,e));return a*a*(3-2*a)}kickFlash(e){this.flash=Math.min(Ke,this.flash+e)}kickSpark(){this.spark=Math.min(Xe,this.spark+qe),this.sparkSeed++}advanceGridMode(){if(this.pinnedGridMode!==null)return;let e=this.gridMode;for(;e===this.gridMode;)e=Math.floor(this.rand()*Q);this.gridMode=e,this.gridModeTimer=ee+this.rand()*te,this.gridModePending=!1,this.gridModePendingElapsed=0}updateFx(e,a){const t=this.material.uniforms;if(this.pinnedFx!==null){t.uFxMode.value=this.pinnedFx,t.uFxAmt.value=a.fxAmount;return}if(!this.fxActive&&(this.fxTimeToNext-=e,this.fxTimeToNext<=0)){this.fxActive=!0,this.fxAge=0,this.fxDuration=St+this.rand()*At;let i=this.fxMode;for(;i===this.fxMode;)i=Math.floor(this.rand()*se);this.fxMode=i}if(this.fxActive){this.fxAge+=e;const i=this.sst(Math.min(1,this.fxAge/.9)),s=1-this.sst(Math.max(0,(this.fxAge-(this.fxDuration-1.2))/1.2));t.uFxAmt.value=i*s*a.fxAmount,this.fxAge>=this.fxDuration&&(this.fxActive=!1,this.fxTimeToNext=ae+this.rand()*ie)}else t.uFxAmt.value=0;t.uFxMode.value=this.fxMode}startScan(e,a,t){let i=this.scanSlots.findIndex(r=>!r.active);if(i<0){i=0;let r=this.scanSlots[0].age;for(let h=1;h<this.scanSlots.length;h++)this.scanSlots[h].age>r&&(r=this.scanSlots[h].age,i=h)}const s=this.scanSlots[i];s.active=!0,s.age=0;const o=this.forceMisAlways||this.rand()<t.misProb;s.mislabel=o,this.scanA[i].set(e,a,0,1),this.scanB[i].set(it+this.rand()*at,o?1:0,this.rand(),0)}pickVisiblePoint(e){const a=this.cover,t=this.material.uniforms.uZoom.value;let i=0,s=0;const o=e.survivorFocus>.5;for(let u=0;u<4&&(i=(this.rand()-.5)*.85,s=(this.rand()-.5)*.85,!(!o||Math.hypot(i,s)>.3));u++);const r=.5+this.pan.x+i*a.x/t,h=.5+this.pan.y+s*a.y/t;return{x:r,y:h}}fireScan(e){const{x:a,y:t}=this.pickVisiblePoint(e);this.startScan(a-Math.floor(a),t-Math.floor(t),e)}scheduleScans(e,a,t){const i=Math.max(0,a);if(i<=0){this.scanScheduleRate=0;return}for(this.scanScheduleRate>0&&this.scanTimeToNext>0?this.scanTimeToNext=Math.min(T,this.scanTimeToNext*(this.scanScheduleRate/i)):this.scanScheduleRate<=0&&(this.scanTimeToNext=0),this.scanScheduleRate=i,this.scanTimeToNext-=e;this.scanTimeToNext<=0;)this.fireScan(t),this.kickFlash(We),this.beatCount++,this.beatFlash=1,this.scanTimeToNext+=F(this.rand,i),this.scanScheduleRate=i}ageScans(e,a){for(let t=0;t<this.scanSlots.length;t++){const i=this.scanSlots[t];if(!i.active)continue;const s=i.age;i.age+=e;const o=i.age;if(this.scanA[t].z=o,s<L&&o>=L&&(this.classified+=st),o>=tt)i.active=!1,this.scanA[t].w=0,this.field.stamps[t].w=0;else{const r=this.scanB[t].x,h=o>=et&&o<L;this.field.stamps[t].set(this.scanA[t].x,this.scanA[t].y,r/a.commFreq,h?a.scanDrain:0)}}}fireLink(e){let a=this.linkSlots.findIndex(u=>!u.active);if(a<0){a=0;let u=this.linkSlots[0].age;for(let d=1;d<this.linkSlots.length;d++)this.linkSlots[d].age>u&&(u=this.linkSlots[d].age,a=d)}const t=this.pickVisiblePoint(e);let i=this.pickVisiblePoint(e),s=i.x-t.x,o=i.y-t.y;Math.hypot(s,o)>.35&&(i=this.pickVisiblePoint(e),s=i.x-t.x,o=i.y-t.y);const r=Math.hypot(s,o);if(r>.35){const u=.35/r;i={x:t.x+s*u,y:t.y+o*u}}const h=this.linkSlots[a];h.active=!0,h.struck=!1,h.age=0,this.linkA[a].set(t.x,t.y,0,1),this.linkB[a].set(i.x,i.y,this.rand(),0)}scheduleLinks(e,a,t){const i=Math.max(0,a);if(i<=0){this.linkScheduleRate=0;return}for(this.linkScheduleRate>0&&this.linkTimeToNext>0?this.linkTimeToNext=Math.min(T,this.linkTimeToNext*(this.linkScheduleRate/i)):this.linkScheduleRate<=0&&(this.linkTimeToNext=0),this.linkScheduleRate=i,this.linkTimeToNext-=e;this.linkTimeToNext<=0;)this.fireLink(t),this.linkTimeToNext+=F(this.rand,i),this.linkScheduleRate=i}ageLinks(e,a){for(let t=0;t<this.linkSlots.length;t++){const i=this.linkSlots[t];if(!i.active)continue;const s=i.age;i.age+=e;const o=i.age;if(this.linkA[t].z=o,!i.struck&&s<Y&&o>=Y){i.struck=!0;const r=this.linkB[t].x,h=this.linkB[t].y,u=r-Math.floor(r),d=h-Math.floor(h);this.startScan(u,d,a),this.fireKill(u,d),this.kickFlash(D)}o>=ct&&(i.active=!1,this.linkA[t].w=0)}}scriptedMassScan(){this.pendingBurst=this.stampSlotCount,this.burstTimer=0,this.startWave(),this.kickFlash(I)}scriptedDrop(){this.kickFlash(I),this.gridSlam=gt,this.startWave()}startWave(){this.waveCx=this.rand(),this.waveCy=this.rand(),this.waveActive=!0,this.waveAge=0}scheduleWaves(e,a){if(this.waveActive)if(this.waveAge+=e,this.waveAge>=O)this.waveActive=!1,this.field.wave.w=0;else{const i=this.sst(this.waveAge/O);this.field.wave.set(this.waveCx,this.waveCy,i*lt,nt*(1-this.waveAge/O))}const t=Math.max(0,a)/60;if(!(t<=0))for(this.waveTimeToNext-=e;this.waveTimeToNext<=0;){this.startWave();const i=Math.max(1e-6,this.rand());this.waveTimeToNext+=-Math.log(i)/t}}fireRunner(){let e=this.runnerSlots.findIndex(o=>!o.active);if(e<0){e=0;let o=this.runnerSlots[0].age;for(let r=1;r<this.runnerSlots.length;r++)this.runnerSlots[r].age>o&&(o=this.runnerSlots[r].age,e=r)}const a=this.cover,t=this.rand()<.5?0:1,i=t===0?(this.rand()-.5)*a.y:(this.rand()-.5)*a.x,s=this.runnerSlots[e];s.active=!0,s.age=0,this.runnerValues[e].set(t,i,0,1)}ageRunners(e){for(let a=0;a<this.runnerSlots.length;a++){const t=this.runnerSlots[a];t.active&&(t.age+=e,t.age>=pt?(t.active=!1,this.runnerValues[a].w=0):this.runnerValues[a].z=t.age)}}scheduleRunners(e,a){const t=Math.max(0,a);if(t<=0){this.runnerScheduleRate=0;return}for(this.runnerScheduleRate>0&&this.runnerTimeToNext>0?this.runnerTimeToNext=Math.min(T,this.runnerTimeToNext*(this.runnerScheduleRate/t)):this.runnerScheduleRate<=0&&(this.runnerTimeToNext=0),this.runnerScheduleRate=t,this.runnerTimeToNext-=e;this.runnerTimeToNext<=0;)this.fireRunner(),this.runnerTimeToNext+=F(this.rand,t),this.runnerScheduleRate=t}fireKill(e,a){let t=this.killSlots.findIndex(s=>!s.active);if(t<0){t=0;let s=this.killSlots[0].age;for(let o=1;o<this.killSlots.length;o++)this.killSlots[o].age>s&&(s=this.killSlots[o].age,t=o)}const i=this.killSlots[t];i.active=!0,i.age=0,this.killValues[t].set(e,a,0,1)}ageKills(e){for(let a=0;a<this.killSlots.length;a++){const t=this.killSlots[a];t.active&&(t.age+=e,t.age>=ut?(t.active=!1,this.killValues[a].w=0):this.killValues[a].z=t.age)}}activatePoke(e,a){let t=this.pokeSlots.findIndex(s=>!s.active);t<0&&(t=0);const i=this.pokeSlots[t];i.active=!0,i.age=0,this.field.pokes[t].set(e,a,Lt,Ot)}updatePokeAges(e){for(let a=0;a<this.pokeSlots.length;a++){const t=this.pokeSlots[a];t.active&&(t.age+=e,t.age>=It&&(t.active=!1,this.field.pokes[a].w=0))}}activateRipple(e,a){let t=this.rippleSlots.findIndex(s=>!s.active);t<0&&(t=0);const i=this.rippleSlots[t];i.active=!0,i.age=0,this.rippleValues[t].set(e-Math.floor(e),a-Math.floor(a),0,1)}updateRippleAges(e){for(let a=0;a<this.rippleSlots.length;a++){const t=this.rippleSlots[a];t.active&&(t.age+=e,t.age>=je?(t.active=!1,this.rippleValues[a].w=0):this.rippleValues[a].z=t.age)}}warmup(e,a,t){this.field.clearField(),this.field.setActParams(e);const i=this.pinnedClassified??e.classifiedFloor;this.field.seedField(i,e.commFreq,t),this.classified=i;for(let s=0;s<a;s++)this.scheduleScans(C,e.scanRate,e),this.ageScans(C,e),this.field.step(C,1)}update(e,a){const t=Ge(a.time),i=t.actIndex===3||t.actIndex===4?f[t.actIndex]:t.params;this.lastDt=e;const s=this.pinnedOrder??Ie(a.time).order;this.firstUpdate&&(this.firstUpdate=!1,this.warmup(i,Se,s)),this.lastSongTime>=0&&a.time<this.lastSongTime-10&&this.warmup(i,Ae,s),this.lastSongTime>=0&&a.time-this.lastSongTime>=0&&a.time-this.lastSongTime<.5&&(this.lastSongTime<j&&a.time>=j&&this.scriptedMassScan(),this.lastSongTime<$&&a.time>=$&&this.scriptedDrop(),this.lastSongTime<Z&&a.time>=Z&&this.kickFlash(I),this.lastSongTime<J&&a.time>=J&&(this.flickerEnv=1)),this.lastSongTime=a.time;const o=this.bassE,r=this.highE,h=Math.min(1,e*8);if(this.bassE+=(a.bass-this.bassE)*h,this.midE+=(a.mid-this.midE)*h,this.highE+=(a.high-this.highE)*h,this.bassSlowE+=(this.bassE-this.bassSlowE)*Math.min(1,e*Ne),this.highSlowE+=(this.highE-this.highSlowE)*Math.min(1,e*Ve),this.onsetCooldown-=e,this.scanOnsetCooldown-=e,this.linkOnsetCooldown-=e,this.onsetCooldown<=0&&this.bassE>o&&this.bassE>Be&&this.bassE>this.bassSlowE*(1+He)&&(this.onsetCooldown=P,this.scanOnsetCooldown<=0&&i.scanRate>.5&&(this.fireScan(i),this.kickFlash(D),this.scanOnsetCooldown=Math.max(P,60/i.scanRate)),this.beatCount++,this.beatFlash=1,this.onsetCount++,this.onsetCount%2===0?t.actIndex===4&&!this.waveActive&&this.startWave():this.linkOnsetCooldown<=0&&i.linkRate>.5&&(this.fireLink(i),this.linkOnsetCooldown=Math.max(P,60/i.linkRate)),this.beatPulse=Math.min(xt,this.beatPulse+wt),this.gridModePending&&this.advanceGridMode()),this.highOnsetCooldown-=e,this.runnerOnsetCooldown-=e,this.highOnsetCooldown<=0&&this.highE>r&&this.highE>ze&&this.highE>this.highSlowE*(1+Ue)&&(this.highOnsetCooldown=q,this.kickSpark(),this.runnerOnsetCooldown<=0&&i.runnerRate>.5&&(this.fireRunner(),this.runnerOnsetCooldown=Math.max(q,60/i.runnerRate))),this.forceSparkAlways&&(this.sparkDebugTimer-=e,this.sparkDebugTimer<=0&&(this.kickSpark(),this.sparkDebugTimer=Ye)),this.spark*=Math.exp(-7*e),this.forceScanAlways&&(this.scanDebugTimer-=e,this.scanDebugTimer<=0&&(this.fireScan(i),this.scanDebugTimer=ot)),this.scheduleScans(e,i.scanRate,i),this.scheduleWaves(e,i.waveRate),this.forceWaveAlways&&(this.waveDebugTimer-=e,this.waveDebugTimer<=0&&(this.startWave(),this.waveDebugTimer=3)),this.scheduleLinks(e,i.linkRate,i),this.ageLinks(e,i),this.forceLinkAlways&&(this.linkDebugTimer-=e,this.linkDebugTimer<=0&&(this.fireLink(i),this.linkDebugTimer=ht)),this.scheduleRunners(e,i.runnerRate),this.ageRunners(e),this.forceRunAlways&&(this.runDebugTimer-=e,this.runDebugTimer<=0&&(this.fireRunner(),this.runDebugTimer=vt)),this.ageKills(e),this.forceKillAlways&&(this.killDebugTimer-=e,this.killDebugTimer<=0)){const{x:l,y:m}=this.pickVisiblePoint(i);this.fireKill(l-Math.floor(l),m-Math.floor(m)),this.killDebugTimer=ft}if(this.forcePopAlways&&(this.popDebugTimer-=e,this.popDebugTimer<=0&&(this.beatFlash=1,this.beatCount++,this.popDebugTimer=bt)),this.pendingBurst>0)for(this.burstTimer-=e;this.burstTimer<=0&&this.pendingBurst>0;)this.fireScan(i),this.pendingBurst--,this.burstTimer+=.18;this.pinnedClassified!==null?this.classified=this.pinnedClassified:this.classified=Math.max(this.classified,i.classifiedFloor),this.ageScans(e,i),this.updatePokeAges(e),this.updateRippleAges(e),this.classified=Math.min(1,this.classified),this.pinnedClassified!==null&&(this.classified=this.pinnedClassified),this.flash*=Math.exp(-3.4*e),this.gridSlam*=Math.exp(-2.5*e),this.beatPulse*=Math.exp(-4*e),this.beatFlash*=Math.exp(-2.8*e),this.updateFx(e,i),this.pinnedGridMode===null&&(this.gridModeTimer-=e,this.gridModeTimer<=0&&!this.gridModePending&&(this.gridModePending=!0,this.gridModePendingElapsed=0),this.gridModePending&&(this.gridModePendingElapsed+=e,this.gridModePendingElapsed>=yt&&this.advanceGridMode())),this.forceFlickerAlways?this.flickerEnv=1:this.flickerEnv*=Math.exp(-.6*e);const u=this.forceLifeFast?kt:1;this.lifeClock+=e*i.lifeRate*u*(1+this.beatPulse*3),this.field.setVitalityMod(1+this.bassE*.5),this.field.setActParams(i);let d;if(t.actIndex===0){const l=p[1]-p[0],m=t.localT*l,g=this.sst(Math.min(1,m/Mt)),G=oe+(f[0].zoom-oe)*g;d=G+(f[1].zoom-G)*t.blend,this.userPanned||this.pan.set(ce.x*(1-g),ce.y*(1-g))}else if(t.actIndex===2)d=f[2].zoom;else if(t.actIndex===3){const l=p[4]-p[3],m=t.localT*l,g=this.sst(Math.min(1,m/Tt));d=f[2].zoom+(f[3].zoom-f[2].zoom)*g}else if(t.actIndex===4){const l=p[5]-p[4],m=t.localT*l;m<le?d=f[3].zoom+(f[4].zoom-f[3].zoom)*this.sst(m/le):d=i.zoom}else d=i.zoom;const w=Rt*(1-.7*i.survivorFocus);d*=1+w*Math.sin(a.time*Ct+this.breathPhase);const n=this.material.uniforms;n.uTime.value+=e,n.uZoom.value=d,n.uCommFreq.value=i.commFreq,n.uChurn.value=i.churn,n.uChatter.value=i.chatterRate*(1+this.midE*.8),n.uWarmth.value=i.warmth,n.uSparkle.value=this.highE,n.uGlyphKick.value=this.spark,n.uGlyphSeed.value=this.sparkSeed,n.uEnergy.value=De(a.time).energy,n.uFlash.value=this.flash,n.uClassified.value=this.classified,n.uDesat.value=Math.min(1,this.classified*.6),n.uMachineFrac.value=i.machineFrac,n.uMachineOrder.value=s,n.uGridStrength.value=i.gridStrength,n.uGridFine.value=i.gridFine,n.uGlyphDensity.value=i.glyphDensity,n.uHueSat.value=i.hueSat,n.uRustMix.value=i.rustMix,n.uInkPersist.value=i.inkPersist,n.uVignette.value=i.vignette,n.uMotes.value=i.motes,n.uGroundLight.value=i.groundLight,n.uSurvivorFocus.value=i.survivorFocus,n.uGridSlam.value=this.gridSlam,n.uFlicker.value=this.flickerEnv,n.uLifeClock.value=this.lifeClock,n.uPresence.value=i.presence,n.uWriggle.value=i.wriggle,n.uDrift.value=i.drift,n.uEventVivid.value=i.eventVivid,n.uBeatCount.value=this.beatCount,n.uBeatFlash.value=this.beatFlash,n.uBeatPop.value=i.beatPop,n.uGridLife.value=i.gridLife,n.uGridMode.value=this.pinnedGridMode??this.gridMode,n.uAura.value=this.forceAuraAlways?1:i.aura,n.uAuraPulse.value=this.bassE;const y=this.cover;if(this.held){if(e>1e-5){const l=Math.min(1,e*Dt),m=Math.min(S,Math.max(-S,this.dragDx/e)),g=Math.min(S,Math.max(-S,this.dragDy/e));this.velX+=(m-this.velX)*l,this.velY+=(g-this.velY)*l}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*y.x/d*e,this.pan.y+=this.velY*y.y/d*e;const l=Math.exp(-2.5*e);this.velX*=l,this.velY*=l,Math.abs(this.velX)<he&&(this.velX=0),Math.abs(this.velY)<he&&(this.velY=0)}const x=Math.hypot(this.pan.x,this.pan.y);x>_&&(this.pan.x*=_/x,this.pan.y*=_/x,this.velX=0,this.velY=0)}pointer(e){const a=this.material.uniforms.uZoom.value,t=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const i=(e.x-.5)*t.x/a+.5+this.pan.x,s=(e.y-.5)*t.y/a+.5+this.pan.y,o=i-Math.floor(i),r=s-Math.floor(s);this.activatePoke(o,r),this.activateRipple(o,r),this.kickFlash(D);return}if(e.type==="move"){if(!this.held)return;this.userPanned=!0,this.pan.x+=e.dx*t.x/a,this.pan.y+=e.dy*t.y/a,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.field.step(this.lastDt,this.stepsPerFrame),this.material.uniforms.uField.value=this.field.texture,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}resize(e,a){if(!this.material||e<=0||a<=0)return;const t=Math.min(3.5,Math.max(.28,e/a));t>=1?this.cover.set(t,1):this.cover.set(1,1/t),this.material.uniforms.uCover.value.copy(this.cover)}dispose(){this.field.dispose(),this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const Gt={default:()=>new _t},Bt=Gt.default;export{Bt as default};
//# sourceMappingURL=index-BZ9qhX4c.js.map
