import{a as k,R as A,L as D,b as j,H as Z,V as M,c as T,S as W,O as X,d as K,e as F,M as Y}from"./three-vlqji54k.js";import{m as $}from"./random-DL1jLgMw.js";const J=1024,Q=512,_=256,C=6,ee=4,G=3,te=2,ie=150,se=60,y=1/30,P=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`}const d=[0,34,78,120,152,188,204,244,259.835],u=[{name:"seed",ignitionRate:14,diff:.35,eps:11,exA:.7,exB:.02,vRate:1,drive:0,cellFreq:8,zoom:1.35,bloomGain:.55,sat:.7,frontGain:.5,refractGlow:.25,filament:.35,microTex:.2,warmth:.05,suppress:0,dust:.25},{name:"first-chains",ignitionRate:42,diff:.45,eps:12,exA:.76,exB:.02,vRate:1.1,drive:0,cellFreq:9,zoom:1.2,bloomGain:.75,sat:.82,frontGain:.7,refractGlow:.35,filament:.55,microTex:.35,warmth:.1,suppress:0,dust:.3},{name:"wiring-up",ignitionRate:78,diff:.55,eps:13,exA:.82,exB:.02,vRate:1.2,drive:0,cellFreq:10,zoom:1.08,bloomGain:.9,sat:.9,frontGain:.85,refractGlow:.45,filament:.7,microTex:.5,warmth:.2,suppress:0,dust:.3},{name:"synchrony",ignitionRate:70,diff:.7,eps:15,exA:.9,exB:.02,vRate:1.3,drive:.006,cellFreq:17,zoom:.72,bloomGain:1,sat:1,frontGain:1,refractGlow:.55,filament:.95,microTex:.6,warmth:.3,suppress:0,dust:.35},{name:"strain",ignitionRate:95,diff:.6,eps:14,exA:.86,exB:.03,vRate:1.2,drive:0,cellFreq:15,zoom:.85,bloomGain:.85,sat:.8,frontGain:.8,refractGlow:.6,filament:.75,microTex:.45,warmth:.6,suppress:0,dust:.4},{name:"fraying",ignitionRate:46,diff:.45,eps:12,exA:.72,exB:.05,vRate:1.1,drive:0,cellFreq:14,zoom:.92,bloomGain:.65,sat:.68,frontGain:.6,refractGlow:.5,filament:.5,microTex:.35,warmth:.55,suppress:0,dust:.45},{name:"collapse",ignitionRate:34,diff:.5,eps:13,exA:.82,exB:.02,vRate:1.3,drive:0,cellFreq:12,zoom:1,bloomGain:.85,sat:.72,frontGain:.7,refractGlow:.5,filament:.55,microTex:.4,warmth:.5,suppress:1,dust:.4},{name:"cold-lattice",ignitionRate:5,diff:.3,eps:10,exA:.66,exB:.04,vRate:1,drive:0,cellFreq:9,zoom:1.3,bloomGain:.45,sat:.55,frontGain:.45,refractGlow:.3,filament:.3,microTex:.15,warmth:.15,suppress:0,dust:.25}],v=[[0,.1],[30,.18],[33.8,.2],[34.3,.46],[76,.52],[78,.5],[116,.8],[119.8,.84],[120.3,.98],[138,1],[152,.9],[188,.74],[203.6,.66],[204.3,.26],[230,.14],[244,.1],[259.835,.02]],N={energy:0};function he(o){const e=Math.min(Math.max(o,0),v[v.length-1][0]);let t=0;for(;t<v.length-2&&e>=v[t+1][0];)t++;const i=v[t],s=v[t+1],a=Math.min(1,Math.max(0,(e-i[0])/Math.max(.001,s[0]-i[0])));return N.energy=i[1]+(s[1]-i[1])*a,N}const ce=6;function ue(o){const e=Math.min(1,Math.max(0,o));return e*e*(3-2*e)}function fe(o,e,t){if(t<=0)return o;if(t>=1)return e;const i={...o,name:t<.5?o.name:e.name};for(const s of Object.keys(o)){const a=o[s],r=e[s];typeof a=="number"&&typeof r=="number"&&(i[s]=a+(r-a)*t)}return i}function de(o){const e=d[d.length-1],t=Math.min(Math.max(o,0),e-.001);let i=0;for(;i<u.length-1&&t>=d[i+1];)i++;const s=d[i],a=d[i+1]??e,r=Math.min(1,Math.max(0,(t-s)/Math.max(.001,a-s))),n=i<u.length-1,l=a-t,c=n?ue(1-Math.min(1,l/ce)):0,m=u[i],h=n?u[i+1]:m;return{params:fe(m,h,c),actIndex:i,localT:r,blend:c}}const me=.12,pe=.5,E=.35,ve=.18,O=1.2,ge=1.6,xe=.5,Se=.12,x=.028,S=.9,we=1,Te=.045,L=120,U=4,ye=1.2,Ee=.09,Ie=.22,Me=.75,Fe=1.2,Re=.5,B=248,be=9,z=1.1,ke=1.46,Ae=.02,De=.26,f=u.findIndex(o=>o.name==="synchrony"),q=5e-4,_e=10,w=1.5,I=.6,H=d[3],V=d[6],Ce=u.findIndex(o=>o.name==="collapse"),Ge=1.6;class Pe{renderer;scene;camera;field;quad;material;rand;forceIgniteAlways=!1;full=!0;igniteSlotCount=C;stepsPerFrame=G;cover=new T(1,1);pan=new T(0,0);bassE=0;midE=0;highE=0;bassSlowE=0;highSlowE=0;onsetCooldown=0;highOnsetCooldown=0;flash=0;spark=0;sparkSeed=0;forceSparkAlways=!1;sparkDebugTimer=0;igniteTimeToNext=0;igniteSlots=[];rippleSlots=[];rippleValues=[];breathPhase=0;collapseCx=.5;collapseCy=.5;firstUpdate=!0;lastDt=0;lastSongTime=-1;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;init(e){const{renderer:t,seed:i,quality:s}=e;this.renderer=t,this.rand=$(i^2748366359);const a=new URLSearchParams(location.search),n=a.get("solo")==="field"?1:0;this.forceIgniteAlways=a.get("ignite")==="always",this.forceSparkAlways=a.get("spark")==="always",this.full=s.level==="full",this.igniteSlotCount=this.full?C:ee,this.stepsPerFrame=this.full?G:te;for(let c=0;c<this.igniteSlotCount;c++)this.igniteSlots.push({age:0,active:!1});for(let c=0;c<U;c++)this.rippleSlots.push({age:0,active:!1}),this.rippleValues.push(new M(0,0,0,0));this.collapseCx=.4+this.rand()*.2,this.collapseCy=.4+this.rand()*.2,this.breathPhase=this.rand()*Math.PI*2,this.scene=new W,this.camera=new X(-1,1,1,-1,0,1),this.field=new re(t,this.full,this.igniteSlotCount),this.material=new F({vertexShader:le,fragmentShader:ne(U),depthTest:!1,depthWrite:!1,uniforms:{uField:{value:null},uCover:{value:new T(1,1)},uPan:{value:this.pan},uZoom:{value:1},uCellFreq:{value:9},uTime:{value:0},uFlash:{value:0},uSparkle:{value:0},uSparkKick:{value:0},uSparkSeed:{value:0},uRipple:{value:this.rippleValues},uEnergy:{value:0},uBloomGain:{value:.6},uSat:{value:.8},uFrontGain:{value:.6},uRefractGlow:{value:.3},uFilament:{value:.4},uMicroTex:{value:.3},uWarmth:{value:0},uDust:{value:.3},uSoloMode:{value:n}}}),this.quad=new Y(new K(2,2),this.material),this.scene.add(this.quad);const l=t.domElement;this.resize(l.clientWidth||1,l.clientHeight||1)}kickFlash(e){this.flash=Math.min(ge,this.flash+e)}ignite(e,t,i,s){let a=this.igniteSlots.findIndex(c=>!c.active);a<0&&(a=0);const r=this.igniteSlots[a];r.active=!0,r.age=0;const n=e-Math.floor(e),l=t-Math.floor(t);this.field.seeds[a].set(n,l,i,s)}updateIgniteAges(e){for(let t=0;t<this.igniteSlots.length;t++){const i=this.igniteSlots[t];i.active&&(i.age+=e,i.age>=Se&&(i.active=!1,this.field.seeds[t].w=0))}}activateRipple(e,t){let i=this.rippleSlots.findIndex(a=>!a.active);i<0&&(i=0);const s=this.rippleSlots[i];s.active=!0,s.age=0,this.rippleValues[i].set(e-Math.floor(e),t-Math.floor(t),0,1)}updateRippleAges(e){for(let t=0;t<this.rippleSlots.length;t++){const i=this.rippleSlots[t];i.active&&(i.age+=e,i.age>=ye?(i.active=!1,this.rippleValues[t].w=0):this.rippleValues[t].z=i.age)}}kickSpark(){this.spark=Math.min(Fe,this.spark+Me),this.sparkSeed++}scriptedFinalBlip(){const e=.35+this.rand()*.3,t=.35+this.rand()*.3;this.ignite(e,t,x*1.2,S),this.kickFlash(E)}sst(e){const t=Math.min(1,Math.max(0,e));return t*t*(3-2*t)}scheduleIgnitions(e,t){const i=Math.max(0,t)/60;if(!(i<=0))for(this.igniteTimeToNext-=e;this.igniteTimeToNext<=0;){this.ignite(this.rand(),this.rand(),x,S),this.kickFlash(ve);const s=Math.max(1e-6,this.rand());this.igniteTimeToNext+=-Math.log(s)/i}}scriptedSynchronyHit(){const e=this.full?5:3;for(let t=0;t<e;t++)this.ignite(this.rand(),this.rand(),x*1.6,S);this.kickFlash(O)}warmup(e,t){this.field.clearField(),this.field.setActParams(e);for(let i=0;i<t;i++)this.scheduleIgnitions(y,this.forceIgniteAlways?L:e.ignitionRate),this.field.step(y,1),this.updateIgniteAges(y)}update(e,t){const i=de(t.time),s=i.params;this.lastDt=e,this.firstUpdate&&(this.firstUpdate=!1,this.warmup(s,ie)),this.lastSongTime>=0&&t.time<this.lastSongTime-10&&this.warmup(s,se),this.lastSongTime>=0&&t.time-this.lastSongTime>=0&&t.time-this.lastSongTime<.5&&(this.lastSongTime<H&&t.time>=H&&this.scriptedSynchronyHit(),this.lastSongTime<V&&t.time>=V&&this.kickFlash(O),this.lastSongTime<B&&t.time>=B&&this.scriptedFinalBlip()),this.lastSongTime=t.time;const a=Math.min(1,e*8);if(this.bassE+=(t.bass-this.bassE)*a,this.midE+=(t.mid-this.midE)*a,this.highE+=(t.high-this.highE)*a,this.bassSlowE+=(t.bass-this.bassSlowE)*Math.min(1,e*1.5),this.highSlowE+=(t.high-this.highSlowE)*Math.min(1,e*1.5),this.onsetCooldown-=e,this.onsetCooldown<=0&&this.bassE-this.bassSlowE>me&&(this.ignite(this.rand(),this.rand(),x,S),this.kickFlash(E),this.onsetCooldown=pe),this.highOnsetCooldown-=e,this.highOnsetCooldown<=0&&this.highE-this.highSlowE>Ee&&(this.kickSpark(),this.highOnsetCooldown=Ie),this.forceSparkAlways&&(this.sparkDebugTimer-=e,this.sparkDebugTimer<=0&&(this.kickSpark(),this.sparkDebugTimer=Re)),this.spark*=Math.exp(-7*e),this.scheduleIgnitions(e,this.forceIgniteAlways?L:s.ignitionRate),this.updateIgniteAges(e),this.updateRippleAges(e),i.actIndex===Ce){const h=i.localT*Ge;this.field.suppress.set(this.collapseCx,this.collapseCy,h,s.suppress)}else this.field.suppress.w=0;this.flash*=Math.exp(-3.4*e),this.field.setDiffMod(1+this.midE*xe),this.field.setActParams(s);let r=s.zoom,n=s.cellFreq;if(i.actIndex===0){const h=z+(ke-z)*this.sst(i.localT);r=h+(u[1].zoom-h)*i.blend}else if(i.actIndex===f-1)r=u[f-1].zoom,n=u[f-1].cellFreq;else if(i.actIndex===f){const h=d[f+1]-d[f],g=this.sst(Math.min(1,i.localT*h/be)),p=u[f-1],R=u[f],b=u[f+1];r=p.zoom+(R.zoom-p.zoom)*g,n=p.cellFreq+(R.cellFreq-p.cellFreq)*g,r+=(b.zoom-r)*i.blend,n+=(b.cellFreq-n)*i.blend}r*=1+Ae*Math.sin(t.time*De+this.breathPhase);const l=this.material.uniforms;l.uTime.value+=e,l.uZoom.value=r,l.uCellFreq.value=n,l.uBloomGain.value=s.bloomGain,l.uSat.value=s.sat,l.uFrontGain.value=s.frontGain,l.uRefractGlow.value=s.refractGlow,l.uFilament.value=s.filament,l.uMicroTex.value=s.microTex,l.uWarmth.value=s.warmth,l.uDust.value=s.dust,l.uFlash.value=this.flash,l.uSparkle.value=this.highE,l.uSparkKick.value=this.spark,l.uSparkSeed.value=this.sparkSeed,l.uEnergy.value=he(t.time).energy;const c=this.cover;if(this.held){if(e>1e-5){const h=Math.min(1,e*_e),g=Math.min(w,Math.max(-w,this.dragDx/e)),p=Math.min(w,Math.max(-w,this.dragDy/e));this.velX+=(g-this.velX)*h,this.velY+=(p-this.velY)*h}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*c.x/r*e,this.pan.y+=this.velY*c.y/r*e;const h=Math.exp(-2.5*e);this.velX*=h,this.velY*=h,Math.abs(this.velX)<q&&(this.velX=0),Math.abs(this.velY)<q&&(this.velY=0)}const m=Math.hypot(this.pan.x,this.pan.y);m>I&&(this.pan.x*=I/m,this.pan.y*=I/m,this.velX=0,this.velY=0)}pointer(e){const t=this.material.uniforms.uZoom.value,i=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const s=(e.x-.5)*i.x/t+.5+this.pan.x,a=(e.y-.5)*i.y/t+.5+this.pan.y;this.ignite(s,a,Te,we),this.activateRipple(s,a),this.kickFlash(E);return}if(e.type==="move"){if(!this.held)return;this.pan.x+=e.dx*i.x/t,this.pan.y+=e.dy*i.y/t,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.field.step(this.lastDt,this.stepsPerFrame),this.material.uniforms.uField.value=this.field.texture,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}resize(e,t){if(!this.material||e<=0||t<=0)return;const i=Math.min(3.5,Math.max(.28,e/t));i>=1?this.cover.set(i,1):this.cover.set(1,1/i),this.material.uniforms.uCover.value.copy(this.cover)}dispose(){this.field.dispose(),this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const Ne={default:()=>new Pe},Ue=Ne.default;export{Ue as default};
//# sourceMappingURL=index-BN5mrn_i.js.map
