import{a as I,R as A,L as C,b as J,H as Z,V as E,c as T,S as V,O as K,d as X,e as k,M as Y}from"./three-vlqji54k.js";import{m as Q}from"./random-DL1jLgMw.js";const $=1024,j=512,_=256,D=6,ee=4,L=3,te=2,ie=300,ae=60,R=1/30,P=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
uniform vec4 uSeeds[${l}]; // xy pos, z radius (uv), w strength (0 = inactive)
...
uniform vec4 uRipple[${l}]; // tap ripples: xy field pos, z age (s), w strength (0 = inactive)
...
`}const d=[0,34,78,120,152,188,204,244,259.835],u=[{name:"seed",ignitionRate:14,diff:.35,eps:11,exA:.7,exB:.02,vRate:1,drive:0,cellFreq:8,zoom:1.35,bloomGain:.55,sat:.7,frontGain:.5,refractGlow:.4,filament:.35,microTex:.2,warmth:.05,suppress:0,dust:.25,rewireRate:0,rewireJump:0,rewireCrack:0,cellLife:.4},{name:"first-chains",ignitionRate:80,diff:.45,eps:12,exA:.8,exB:.02,vRate:1.1,drive:0,cellFreq:9,zoom:1.2,bloomGain:.75,sat:.82,frontGain:.7,refractGlow:.5,filament:.55,microTex:.35,warmth:.1,suppress:0,dust:.3,rewireRate:.6,rewireJump:.25,rewireCrack:.1,cellLife:.55},{name:"wiring-up",ignitionRate:110,diff:.65,eps:13,exA:.88,exB:.02,vRate:1.2,drive:0,cellFreq:10,zoom:1.08,bloomGain:.9,sat:.9,frontGain:.85,refractGlow:.6,filament:.7,microTex:.5,warmth:.2,suppress:0,dust:.3,rewireRate:1,rewireJump:.45,rewireCrack:.3,cellLife:.75},{name:"synchrony",ignitionRate:70,diff:.7,eps:15,exA:.9,exB:.02,vRate:1.3,drive:.006,cellFreq:17,zoom:.72,bloomGain:1,sat:1,frontGain:1,refractGlow:.7,filament:.95,microTex:.6,warmth:.3,suppress:0,dust:.35,rewireRate:1.5,rewireJump:.8,rewireCrack:.7,cellLife:1},{name:"strain",ignitionRate:95,diff:.6,eps:14,exA:.86,exB:.03,vRate:1.2,drive:0,cellFreq:15,zoom:.85,bloomGain:.85,sat:.88,frontGain:.8,refractGlow:.75,filament:.75,microTex:.45,warmth:.6,suppress:0,dust:.4,rewireRate:1.4,rewireJump:.75,rewireCrack:.85,cellLife:.95},{name:"fraying",ignitionRate:70,diff:.45,eps:12,exA:.78,exB:.05,vRate:1.1,drive:0,cellFreq:14,zoom:.92,bloomGain:.65,sat:.78,frontGain:.6,refractGlow:.65,filament:.5,microTex:.35,warmth:.55,suppress:0,dust:.45,rewireRate:.8,rewireJump:.5,rewireCrack:.4,cellLife:.6},{name:"collapse",ignitionRate:55,diff:.5,eps:13,exA:.82,exB:.02,vRate:1.3,drive:0,cellFreq:12,zoom:1,bloomGain:.85,sat:.8,frontGain:.7,refractGlow:.65,filament:.55,microTex:.4,warmth:.5,suppress:1,dust:.4,rewireRate:0,rewireJump:0,rewireCrack:0,cellLife:.35},{name:"cold-lattice",ignitionRate:5,diff:.3,eps:10,exA:.66,exB:.04,vRate:1,drive:0,cellFreq:9,zoom:1.3,bloomGain:.45,sat:.55,frontGain:.45,refractGlow:.45,filament:.3,microTex:.15,warmth:.15,suppress:0,dust:.25,rewireRate:0,rewireJump:0,rewireCrack:0,cellLife:.3}],v=[[0,.1],[30,.18],[33.8,.2],[34.3,.46],[76,.52],[78,.5],[116,.8],[119.8,.84],[120.3,.98],[138,1],[152,.9],[188,.74],[203.6,.66],[204.3,.26],[230,.14],[244,.1],[259.835,.02]],G={energy:0};function ce(l){const e=Math.min(Math.max(l,0),v[v.length-1][0]);let t=0;for(;t<v.length-2&&e>=v[t+1][0];)t++;const i=v[t],a=v[t+1],s=Math.min(1,Math.max(0,(e-i[0])/Math.max(.001,a[0]-i[0])));return G.energy=i[1]+(a[1]-i[1])*s,G}const he=6;function ue(l){const e=Math.min(1,Math.max(0,l));return e*e*(3-2*e)}function fe(l,e,t){if(t<=0)return l;if(t>=1)return e;const i={...l,name:t<.5?l.name:e.name};for(const a of Object.keys(l)){const s=l[a],o=e[a];typeof s=="number"&&typeof o=="number"&&(i[a]=s+(o-s)*t)}return i}function de(l){const e=d[d.length-1],t=Math.min(Math.max(l,0),e-.001);let i=0;for(;i<u.length-1&&t>=d[i+1];)i++;const a=d[i],s=d[i+1]??e,o=Math.min(1,Math.max(0,(t-a)/Math.max(.001,s-a))),n=i<u.length-1,r=s-t,h=n?ue(1-Math.min(1,r/he)):0,m=u[i],c=n?u[i+1]:m;return{params:fe(m,c,h),actIndex:i,localT:o,blend:h}}const me=.12,pe=.5,y=.35,ve=.18,O=1.2,ge=1.6,we=.5,xe=.12,w=.028,x=.9,Se=1,Te=.045,N=120,z=4,Re=1.2,ye=.09,be=.22,Ee=.75,ke=1.2,Fe=.5,U=248,Me=9,B=1.1,Ie=1.46,Ae=.02,Ce=.26,f=u.findIndex(l=>l.name==="synchrony"),q=5e-4,_e=10,S=1.5,b=.6,W=d[3],H=d[6],De=u.findIndex(l=>l.name==="collapse"),Le=1.6;class Pe{renderer;scene;camera;field;quad;material;rand;forceIgniteAlways=!1;full=!0;igniteSlotCount=D;stepsPerFrame=L;cover=new T(1,1);pan=new T(0,0);bassE=0;midE=0;highE=0;bassSlowE=0;highSlowE=0;onsetCooldown=0;highOnsetCooldown=0;flash=0;spark=0;sparkSeed=0;forceSparkAlways=!1;forceRewireFast=!1;sparkDebugTimer=0;igniteTimeToNext=0;igniteSlots=[];rippleSlots=[];rippleValues=[];breathPhase=0;collapseCx=.5;collapseCy=.5;firstUpdate=!0;lastDt=0;lastSongTime=-1;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;init(e){const{renderer:t,seed:i,quality:a}=e;this.renderer=t,this.rand=Q(i^2748366359);const s=new URLSearchParams(location.search),o=s.get("solo"),n=o==="field"?1:o==="rewire"?2:0;this.forceIgniteAlways=s.get("ignite")==="always",this.forceSparkAlways=s.get("spark")==="always",this.forceRewireFast=s.get("rewire")==="fast",this.full=a.level==="full",this.igniteSlotCount=this.full?D:ee,this.stepsPerFrame=this.full?L:te;for(let h=0;h<this.igniteSlotCount;h++)this.igniteSlots.push({age:0,active:!1});for(let h=0;h<z;h++)this.rippleSlots.push({age:0,active:!1}),this.rippleValues.push(new E(0,0,0,0));this.collapseCx=.4+this.rand()*.2,this.collapseCy=.4+this.rand()*.2,this.breathPhase=this.rand()*Math.PI*2,this.scene=new V,this.camera=new K(-1,1,1,-1,0,1),this.field=new oe(t,this.full,this.igniteSlotCount),this.material=new k({vertexShader:le,fragmentShader:ne(z),depthTest:!1,depthWrite:!1,uniforms:{uField:{value:null},uCover:{value:new T(1,1)},uPan:{value:this.pan},uZoom:{value:1},uCellFreq:{value:9},uFieldSize:{value:this.full?$:j},uRewireJump:{value:0},uRewireCrack:{value:0},uTime:{value:0},uFlash:{value:0},uSparkle:{value:0},uSparkKick:{value:0},uSparkSeed:{value:0},uRipple:{value:this.rippleValues},uRing:{value:this.field.suppress},uEnergy:{value:0},uBloomGain:{value:.6},uSat:{value:.8},uFrontGain:{value:.6},uRefractGlow:{value:.3},uFilament:{value:.4},uMicroTex:{value:.3},uCellLife:{value:0},uWarmth:{value:0},uDust:{value:.3},uSoloMode:{value:n}}}),this.quad=new Y(new X(2,2),this.material),this.scene.add(this.quad);const r=t.domElement;this.resize(r.clientWidth||1,r.clientHeight||1)}kickFlash(e){this.flash=Math.min(ge,this.flash+e)}ignite(e,t,i,a){let s=this.igniteSlots.findIndex(h=>!h.active);s<0&&(s=0);const o=this.igniteSlots[s];o.active=!0,o.age=0;const n=e-Math.floor(e),r=t-Math.floor(t);this.field.seeds[s].set(n,r,i,a)}updateIgniteAges(e){for(let t=0;t<this.igniteSlots.length;t++){const i=this.igniteSlots[t];i.active&&(i.age+=e,i.age>=xe&&(i.active=!1,this.field.seeds[t].w=0))}}activateRipple(e,t){let i=this.rippleSlots.findIndex(s=>!s.active);i<0&&(i=0);const a=this.rippleSlots[i];a.active=!0,a.age=0,this.rippleValues[i].set(e-Math.floor(e),t-Math.floor(t),0,1)}updateRippleAges(e){for(let t=0;t<this.rippleSlots.length;t++){const i=this.rippleSlots[t];i.active&&(i.age+=e,i.age>=Re?(i.active=!1,this.rippleValues[t].w=0):this.rippleValues[t].z=i.age)}}kickSpark(){this.spark=Math.min(ke,this.spark+Ee),this.sparkSeed++}scriptedFinalBlip(){const e=.35+this.rand()*.3,t=.35+this.rand()*.3;this.ignite(e,t,w*1.2,x),this.kickFlash(y)}sst(e){const t=Math.min(1,Math.max(0,e));return t*t*(3-2*t)}scheduleIgnitions(e,t){const i=Math.max(0,t)/60;if(!(i<=0))for(this.igniteTimeToNext-=e;this.igniteTimeToNext<=0;){this.ignite(this.rand(),this.rand(),w,x),this.kickFlash(ve);const a=Math.max(1e-6,this.rand());this.igniteTimeToNext+=-Math.log(a)/i}}scriptedSynchronyHit(){const e=this.full?5:3;for(let t=0;t<e;t++)this.ignite(this.rand(),this.rand(),w*1.6,x);this.kickFlash(O)}warmup(e,t){this.field.clearField(),this.field.setActParams(e);for(let i=0;i<t;i++)this.scheduleIgnitions(R,this.forceIgniteAlways?N:e.ignitionRate),this.field.step(R,1),this.updateIgniteAges(R)}update(e,t){const i=de(t.time),a=i.params;this.lastDt=e,this.firstUpdate&&(this.firstUpdate=!1,this.warmup(a,ie)),this.lastSongTime>=0&&t.time<this.lastSongTime-10&&this.warmup(a,ae),this.lastSongTime>=0&&t.time-this.lastSongTime>=0&&t.time-this.lastSongTime<.5&&(this.lastSongTime<W&&t.time>=W&&this.scriptedSynchronyHit(),this.lastSongTime<H&&t.time>=H&&this.kickFlash(O),this.lastSongTime<U&&t.time>=U&&this.scriptedFinalBlip()),this.lastSongTime=t.time;const s=Math.min(1,e*8);if(this.bassE+=(t.bass-this.bassE)*s,this.midE+=(t.mid-this.midE)*s,this.highE+=(t.high-this.highE)*s,this.bassSlowE+=(t.bass-this.bassSlowE)*Math.min(1,e*1.5),this.highSlowE+=(t.high-this.highSlowE)*Math.min(1,e*1.5),this.onsetCooldown-=e,this.onsetCooldown<=0&&this.bassE-this.bassSlowE>me&&(this.ignite(this.rand(),this.rand(),w,x),this.kickFlash(y),this.onsetCooldown=pe),this.highOnsetCooldown-=e,this.highOnsetCooldown<=0&&this.highE-this.highSlowE>ye&&(this.kickSpark(),this.highOnsetCooldown=be),this.forceSparkAlways&&(this.sparkDebugTimer-=e,this.sparkDebugTimer<=0&&(this.kickSpark(),this.sparkDebugTimer=Fe)),this.spark*=Math.exp(-7*e),this.scheduleIgnitions(e,this.forceIgniteAlways?N:a.ignitionRate),this.updateIgniteAges(e),this.updateRippleAges(e),i.actIndex===De){const c=i.localT*Le;this.field.suppress.set(this.collapseCx,this.collapseCy,c,a.suppress)}else this.field.suppress.w=0;this.flash*=Math.exp(-3.4*e),this.field.setDiffMod(1+this.midE*we),this.field.setRewireMod(this.forceRewireFast?8:1),this.field.setActParams(a);let o=a.zoom,n=a.cellFreq;if(i.actIndex===0){const c=B+(Ie-B)*this.sst(i.localT);o=c+(u[1].zoom-c)*i.blend}else if(i.actIndex===f-1)o=u[f-1].zoom,n=u[f-1].cellFreq;else if(i.actIndex===f){const c=d[f+1]-d[f],g=this.sst(Math.min(1,i.localT*c/Me)),p=u[f-1],F=u[f],M=u[f+1];o=p.zoom+(F.zoom-p.zoom)*g,n=p.cellFreq+(F.cellFreq-p.cellFreq)*g,o+=(M.zoom-o)*i.blend,n+=(M.cellFreq-n)*i.blend}o*=1+Ae*Math.sin(t.time*Ce+this.breathPhase);const r=this.material.uniforms;r.uTime.value+=e,r.uZoom.value=o,r.uCellFreq.value=n,r.uBloomGain.value=a.bloomGain,r.uSat.value=a.sat,r.uFrontGain.value=a.frontGain,r.uRefractGlow.value=a.refractGlow,r.uFilament.value=a.filament,r.uMicroTex.value=a.microTex,r.uCellLife.value=a.cellLife,r.uWarmth.value=a.warmth,r.uDust.value=a.dust,r.uRewireJump.value=a.rewireJump,r.uRewireCrack.value=a.rewireCrack,r.uFlash.value=this.flash,r.uSparkle.value=this.highE,r.uSparkKick.value=this.spark,r.uSparkSeed.value=this.sparkSeed,r.uEnergy.value=ce(t.time).energy;const h=this.cover;if(this.held){if(e>1e-5){const c=Math.min(1,e*_e),g=Math.min(S,Math.max(-S,this.dragDx/e)),p=Math.min(S,Math.max(-S,this.dragDy/e));this.velX+=(g-this.velX)*c,this.velY+=(p-this.velY)*c}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*h.x/o*e,this.pan.y+=this.velY*h.y/o*e;const c=Math.exp(-2.5*e);this.velX*=c,this.velY*=c,Math.abs(this.velX)<q&&(this.velX=0),Math.abs(this.velY)<q&&(this.velY=0)}const m=Math.hypot(this.pan.x,this.pan.y);m>b&&(this.pan.x*=b/m,this.pan.y*=b/m,this.velX=0,this.velY=0)}pointer(e){const t=this.material.uniforms.uZoom.value,i=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const a=(e.x-.5)*i.x/t+.5+this.pan.x,s=(e.y-.5)*i.y/t+.5+this.pan.y;this.ignite(a,s,Te,Se),this.activateRipple(a,s),this.kickFlash(y);return}if(e.type==="move"){if(!this.held)return;this.pan.x+=e.dx*i.x/t,this.pan.y+=e.dy*i.y/t,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.field.step(this.lastDt,this.stepsPerFrame),this.material.uniforms.uField.value=this.field.texture,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}resize(e,t){if(!this.material||e<=0||t<=0)return;const i=Math.min(3.5,Math.max(.28,e/t));i>=1?this.cover.set(i,1):this.cover.set(1,1/i),this.material.uniforms.uCover.value.copy(this.cover)}dispose(){this.field.dispose(),this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const Ge={default:()=>new Pe},ze=Ge.default;export{ze as default};
//# sourceMappingURL=index-BNwYwolT.js.map
