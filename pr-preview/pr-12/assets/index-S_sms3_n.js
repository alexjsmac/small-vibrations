import{a as L,R as O,L as _,b as de,H as fe,V as v,c as S,S as le,O as ne,d as re,e as A,M as ce}from"./three-vlqji54k.js";import{m as ue}from"./random-DL1jLgMw.js";const me=512,pe=256,G=2,ve=1,H=6,ge=4,M=4,we=240,xe=60,T=1/30,N=256,ke=.05,R=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`}const p=[0,56,128,172,232,316,336.998],m=[{name:"thriving-field",vitalityTarget:.95,churn:.9,presence:.8,lifeRate:.1,wriggle:1,drift:1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.45,fxAmount:.7,aura:0,beatPop:0,eventVivid:0,gridStrength:0,gridFine:0,glyphDensity:.85,chatterRate:1.2,machineFrac:0,classifiedFloor:0,commFreq:6,zoom:1.18,survivorFocus:0,hueSat:1,warmth:.55,rustMix:.05,inkPersist:0,vignette:.3,motes:.15,groundLight:.9},{name:"first-scans",vitalityTarget:.85,churn:.8,presence:.85,lifeRate:.12,wriggle:.8,drift:.8,scanRate:5,scanDrain:.5,misProb:.3,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.7,fxAmount:.8,aura:0,beatPop:.25,eventVivid:.3,gridStrength:.18,gridFine:.25,glyphDensity:.8,chatterRate:1,machineFrac:.08,classifiedFloor:.06,commFreq:6,zoom:1.05,survivorFocus:0,hueSat:.92,warmth:.5,rustMix:.15,inkPersist:.5,vignette:.32,motes:.08,groundLight:.9},{name:"accelerating-catalogue",vitalityTarget:.7,churn:.75,presence:.8,lifeRate:.16,wriggle:.7,drift:.6,scanRate:16,scanDrain:.7,misProb:.15,classifyPressure:0,waveRate:0,linkRate:2,runnerRate:4,gridLife:.7,fxAmount:.6,aura:0,beatPop:.4,eventVivid:.5,gridStrength:.42,gridFine:.5,glyphDensity:.7,chatterRate:.9,machineFrac:.3,classifiedFloor:.28,commFreq:6,zoom:.98,survivorFocus:0,hueSat:.8,warmth:.45,rustMix:.35,inkPersist:.8,vignette:.35,motes:.05,groundLight:.9},{name:"last-unclassified",vitalityTarget:.9,churn:.6,presence:.75,lifeRate:.08,wriggle:1,drift:.7,scanRate:1.5,scanDrain:.4,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.3,fxAmount:.5,aura:1,beatPop:.3,eventVivid:.4,gridStrength:.1,gridFine:.15,glyphDensity:.9,chatterRate:.7,machineFrac:.35,classifiedFloor:.34,commFreq:6,zoom:2.9,survivorFocus:1,hueSat:.85,warmth:.5,rustMix:.3,inkPersist:.5,vignette:.5,motes:.05,groundLight:.9},{name:"total-classification",vitalityTarget:.25,churn:.5,presence:.9,lifeRate:.3,wriggle:.35,drift:.25,scanRate:34,scanDrain:1,misProb:0,classifyPressure:.35,waveRate:10,linkRate:14,runnerRate:22,gridLife:1,fxAmount:.35,aura:0,beatPop:1,eventVivid:1,gridStrength:1,gridFine:1,glyphDensity:.55,chatterRate:1.4,machineFrac:.9,classifiedFloor:.9,commFreq:6,zoom:.82,survivorFocus:0,hueSat:.4,warmth:.35,rustMix:.55,inkPersist:1,vignette:.42,motes:.02,groundLight:.9},{name:"residue",vitalityTarget:.1,churn:.25,presence:.22,lifeRate:.04,wriggle:.15,drift:.1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,gridLife:.3,fxAmount:.5,aura:0,beatPop:.15,eventVivid:.2,gridStrength:.06,gridFine:0,glyphDensity:.08,chatterRate:.3,machineFrac:1,classifiedFloor:.93,commFreq:6,zoom:1.15,survivorFocus:0,hueSat:.2,warmth:.3,rustMix:.45,inkPersist:.35,vignette:.45,motes:.5,groundLight:1}],w=[[0,.22],[30,.34],[36,.28],[54,.3],[56,.36],[100,.45],[126,.5],[128,.58],[167.8,.68],[168.3,.78],[171,.74],[174,.42],[200,.38],[228,.5],[231.7,.55],[232.2,.96],[246,1],[300,.82],[315.6,.7],[316.4,.18],[330,.08],[336.998,.04]],V={energy:0};function Re(h){const e=Math.min(Math.max(h,0),w[w.length-1][0]);let a=0;for(;a<w.length-2&&e>=w[a+1][0];)a++;const t=w[a],i=w[a+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,i[0]-t[0])));return V.energy=t[1]+(i[1]-t[1])*s,V}const x=[[0,0],[128,0],[172,.22],[232,.25],[300,.95],[316,1],[336.998,1]],B={order:0};function Ce(h){const e=Math.min(Math.max(h,0),x[x.length-1][0]);let a=0;for(;a<x.length-2&&e>=x[a+1][0];)a++;const t=x[a],i=x[a+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,i[0]-t[0])));return B.order=t[1]+(i[1]-t[1])*s,B}const Fe=6;function Ee(h){const e=Math.min(1,Math.max(0,h));return e*e*(3-2*e)}function Pe(h,e,a){if(a<=0)return h;if(a>=1)return e;const t={...h,name:a<.5?h.name:e.name};for(const i of Object.keys(h)){const s=h[i],o=e[i];typeof s=="number"&&typeof o=="number"&&(t[i]=s+(o-s)*a)}return t}function De(h){const e=p[p.length-1],a=Math.min(Math.max(h,0),e-.001);let t=0;for(;t<m.length-1&&a>=p[t+1];)t++;const i=p[t],s=p[t+1]??e,o=Math.min(1,Math.max(0,(a-i)/Math.max(.001,s-i))),r=t<m.length-1,d=s-a,c=r?Ee(1-Math.min(1,d/Fe)):0,u=m[t],n=r?m[t+1]:u;return{params:Pe(u,n,c),actIndex:t,localT:o,blend:c}}const Ie=.12,Le=.5,Oe=.09,_e=.22,C=.35,Ge=.18,F=1.2,He=1.6,Ne=.75,Ve=1.2,Be=.5,U=4,Ue=1.2,ze=2,We=1,Ke=4,qe=2,Xe=.5,E=1.4,Ye=2.2,je=.42,$e=.12,Ze=.012,Je=1.2,P=1.6,Qe=.8,et=1,tt=3,z=.35,it=1.6,at=1.6,st=4,ot=8,lt=2,nt=4,rt=.5,ct=.7,W=168,K=232,q=316,X=330,ht=.6,dt=1,ft=1.5,ut=6,mt=.5,Y=4,j=16,$=8,pt=6,Z=14,J=8,vt=5,gt=3,Q=3,ee=9.5,wt=40,xt=8,te=.45,kt=.015,bt=.22;function yt(h,e){const a=h*127.1+e*311.7,t=h*269.5+e*183.3,i=Math.sin(a)*43758.5453,s=Math.sin(t)*43758.5453;return[i-Math.floor(i),s-Math.floor(s)]}const[St,At]=yt(0,0),ie={x:.22+.56*St,y:.22+.56*At},ae=6,se={x:ie.x/ae,y:ie.y/ae},oe=5e-4,Mt=10,y=1.5,D=.6,Tt=.35,Rt=.05,Ct=1;class Ft{renderer;scene;camera;field;quad;material;rand;forceScanAlways=!1;forceMisAlways=!1;forceWaveAlways=!1;forceFlickerAlways=!1;forceSparkAlways=!1;forceLinkAlways=!1;forceRunAlways=!1;forceLifeFast=!1;forceKillAlways=!1;forceAuraAlways=!1;forcePopAlways=!1;pinnedClassified=null;pinnedOrder=null;pinnedGridMode=null;gridMode=0;gridModeTimer=0;gridModePending=!1;gridModePendingElapsed=0;fxMode=0;fxActive=!1;fxAge=0;fxDuration=0;fxTimeToNext=0;pinnedFx=null;full=!0;stampSlotCount=H;stepsPerFrame=G;cover=new S(1,1);pan=new S(0,0);userPanned=!1;bassE=0;midE=0;highE=0;bassSlowE=0;highSlowE=0;onsetCooldown=0;highOnsetCooldown=0;flash=0;beatPulse=0;lifeClock=0;beatCount=0;beatFlash=0;spark=0;sparkSeed=0;sparkDebugTimer=0;scanSlots=[];scanA=[];scanB=[];scanTimeToNext=0;scanDebugTimer=0;pendingBurst=0;burstTimer=0;waveActive=!1;waveAge=0;waveCx=.5;waveCy=.5;waveTimeToNext=0;waveDebugTimer=0;rippleSlots=[];rippleValues=[];pokeSlots=[];linkSlots=[];linkA=[];linkB=[];linkTimeToNext=0;linkDebugTimer=0;onsetCount=0;runnerSlots=[];runnerValues=[];runnerTimeToNext=0;runDebugTimer=0;killSlots=[];killValues=[];killDebugTimer=0;popDebugTimer=0;classified=0;gridSlam=0;flickerEnv=0;breathPhase=0;firstUpdate=!0;lastDt=0;lastSongTime=-1;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;init(e){const{renderer:a,seed:t,quality:i}=e;this.renderer=a,this.rand=ue(t^2999220761);const s=new URLSearchParams(location.search),o=s.get("solo"),r=o==="field"?1:o==="patch"?2:o==="glyphs"?3:o==="machine"?4:0;this.forceScanAlways=s.get("scan")==="always",this.forceMisAlways=s.get("mis")==="always",this.forceWaveAlways=s.get("wave")==="always",this.forceFlickerAlways=s.get("flicker")==="always",this.forceSparkAlways=s.get("spark")==="always",this.forceLinkAlways=s.get("link")==="always",this.forceRunAlways=s.get("run")==="always",this.forceLifeFast=s.get("life")==="fast",this.forceKillAlways=s.get("kill")==="always",this.forcePopAlways=s.get("pop")==="always";const d=s.get("classified");if(d!==null){const l=parseFloat(d);Number.isNaN(l)||(this.pinnedClassified=Math.min(1,Math.max(0,l)))}const c=s.get("order");if(c!==null){const l=parseFloat(c);Number.isNaN(l)||(this.pinnedOrder=Math.min(1,Math.max(0,l)))}const u=s.get("gridmode");if(u!==null){const l=parseInt(u,10);!Number.isNaN(l)&&l>=0&&l<Y&&(this.pinnedGridMode=l)}this.forceAuraAlways=s.get("aura")==="always";const n=s.get("fx");if(n!==null){const l=parseInt(n,10);!Number.isNaN(l)&&l>=0&&l<Q&&(this.pinnedFx=l)}this.full=i.level==="full",this.stampSlotCount=this.full?H:ge;const b=this.full?ze:We,k=this.full?Ke:qe;this.stepsPerFrame=this.full?G:ve;for(let l=0;l<this.stampSlotCount;l++)this.scanSlots.push({age:0,active:!1,mislabel:!1}),this.scanA.push(new v(0,0,0,0)),this.scanB.push(new v(0,0,0,0));for(let l=0;l<U;l++)this.rippleSlots.push({age:0,active:!1}),this.rippleValues.push(new v(0,0,0,0));for(let l=0;l<M;l++)this.pokeSlots.push({age:0,active:!1});for(let l=0;l<tt;l++)this.linkSlots.push({age:0,active:!1,struck:!1}),this.linkA.push(new v(0,0,0,0)),this.linkB.push(new v(0,0,0,0));for(let l=0;l<nt;l++)this.runnerSlots.push({age:0,active:!1}),this.runnerValues.push(new v(0,0,0,0));for(let l=0;l<st;l++)this.killSlots.push({age:0,active:!1}),this.killValues.push(new v(0,0,0,0));this.breathPhase=this.rand()*Math.PI*2,this.pinnedGridMode!==null&&(this.gridMode=this.pinnedGridMode),this.gridModeTimer=j+this.rand()*$,this.fxTimeToNext=J+this.rand()*Z,this.scene=new le,this.camera=new ne(-1,1,1,-1,0,1),this.field=new Ae(a,this.full,this.stampSlotCount),this.material=new A({vertexShader:Me,fragmentShader:Te(U,this.stampSlotCount,b,k),depthTest:!1,depthWrite:!1,uniforms:{uField:{value:null},uTime:{value:0},uCover:{value:new S(1,1)},uZoom:{value:1},uPan:{value:this.pan},uCommFreq:{value:6},uEnergy:{value:0},uFlash:{value:0},uSparkle:{value:0},uGlyphKick:{value:0},uGlyphSeed:{value:0},uChurn:{value:0},uChatter:{value:0},uGlyphDensity:{value:0},uMachineFrac:{value:0},uMachineOrder:{value:0},uClassified:{value:0},uGridStrength:{value:0},uGridFine:{value:0},uGridSlam:{value:0},uHueSat:{value:0},uWarmth:{value:0},uRustMix:{value:0},uInkPersist:{value:0},uVignette:{value:0},uMotes:{value:0},uGroundLight:{value:0},uSurvivorFocus:{value:0},uDesat:{value:0},uSoloMode:{value:r},uFlicker:{value:0},uLifeClock:{value:0},uPresence:{value:0},uWriggle:{value:0},uDrift:{value:0},uWaveVis:{value:this.field.wave},uScanA:{value:this.scanA},uScanB:{value:this.scanB},uRipple:{value:this.rippleValues},uEventVivid:{value:0},uLinkA:{value:this.linkA},uLinkB:{value:this.linkB},uRunner:{value:this.runnerValues},uKill:{value:this.killValues},uBeatCount:{value:0},uBeatFlash:{value:0},uBeatPop:{value:0},uGridLife:{value:0},uGridMode:{value:0},uAura:{value:0},uAuraPulse:{value:0},uFxMode:{value:0},uFxAmt:{value:0}}}),this.quad=new ce(new re(2,2),this.material),this.scene.add(this.quad);const f=a.domElement;this.resize(f.clientWidth||1,f.clientHeight||1)}sst(e){const a=Math.min(1,Math.max(0,e));return a*a*(3-2*a)}kickFlash(e){this.flash=Math.min(He,this.flash+e)}kickSpark(){this.spark=Math.min(Ve,this.spark+Ne),this.sparkSeed++}advanceGridMode(){if(this.pinnedGridMode!==null)return;let e=this.gridMode;for(;e===this.gridMode;)e=Math.floor(this.rand()*Y);this.gridMode=e,this.gridModeTimer=j+this.rand()*$,this.gridModePending=!1,this.gridModePendingElapsed=0}updateFx(e,a){const t=this.material.uniforms;if(this.pinnedFx!==null){t.uFxMode.value=this.pinnedFx,t.uFxAmt.value=a.fxAmount;return}if(!this.fxActive&&(this.fxTimeToNext-=e,this.fxTimeToNext<=0)){this.fxActive=!0,this.fxAge=0,this.fxDuration=vt+this.rand()*gt;let i=this.fxMode;for(;i===this.fxMode;)i=Math.floor(this.rand()*Q);this.fxMode=i}if(this.fxActive){this.fxAge+=e;const i=this.sst(Math.min(1,this.fxAge/.9)),s=1-this.sst(Math.max(0,(this.fxAge-(this.fxDuration-1.2))/1.2));t.uFxAmt.value=i*s*a.fxAmount,this.fxAge>=this.fxDuration&&(this.fxActive=!1,this.fxTimeToNext=J+this.rand()*Z)}else t.uFxAmt.value=0;t.uFxMode.value=this.fxMode}startScan(e,a,t){let i=this.scanSlots.findIndex(r=>!r.active);if(i<0){i=0;let r=this.scanSlots[0].age;for(let d=1;d<this.scanSlots.length;d++)this.scanSlots[d].age>r&&(r=this.scanSlots[d].age,i=d)}const s=this.scanSlots[i];s.active=!0,s.age=0;const o=this.forceMisAlways||this.rand()<t.misProb;s.mislabel=o,this.scanA[i].set(e,a,0,1),this.scanB[i].set(je+this.rand()*$e,o?1:0,this.rand(),0)}pickVisiblePoint(e){const a=this.cover,t=this.material.uniforms.uZoom.value;let i=0,s=0;const o=e.survivorFocus>.5;for(let c=0;c<4&&(i=(this.rand()-.5)*.85,s=(this.rand()-.5)*.85,!(!o||Math.hypot(i,s)>.3));c++);const r=.5+this.pan.x+i*a.x/t,d=.5+this.pan.y+s*a.y/t;return{x:r,y:d}}fireScan(e){const{x:a,y:t}=this.pickVisiblePoint(e);this.startScan(a-Math.floor(a),t-Math.floor(t),e)}scheduleScans(e,a,t){const i=Math.max(0,a)/60;if(!(i<=0))for(this.scanTimeToNext-=e;this.scanTimeToNext<=0;){this.fireScan(t),this.kickFlash(Ge),this.beatCount++,this.beatFlash=1;const s=Math.max(1e-6,this.rand());this.scanTimeToNext+=-Math.log(s)/i}}ageScans(e,a){for(let t=0;t<this.scanSlots.length;t++){const i=this.scanSlots[t];if(!i.active)continue;const s=i.age;i.age+=e;const o=i.age;if(this.scanA[t].z=o,s<E&&o>=E&&(this.classified+=Ze),o>=Ye)i.active=!1,this.scanA[t].w=0,this.field.stamps[t].w=0;else{const r=this.scanB[t].x,d=o>=Xe&&o<E;this.field.stamps[t].set(this.scanA[t].x,this.scanA[t].y,r/a.commFreq,d?a.scanDrain:0)}}}fireLink(e){let a=this.linkSlots.findIndex(c=>!c.active);if(a<0){a=0;let c=this.linkSlots[0].age;for(let u=1;u<this.linkSlots.length;u++)this.linkSlots[u].age>c&&(c=this.linkSlots[u].age,a=u)}const t=this.pickVisiblePoint(e);let i=this.pickVisiblePoint(e),s=i.x-t.x,o=i.y-t.y;Math.hypot(s,o)>.35&&(i=this.pickVisiblePoint(e),s=i.x-t.x,o=i.y-t.y);const r=Math.hypot(s,o);if(r>.35){const c=.35/r;i={x:t.x+s*c,y:t.y+o*c}}const d=this.linkSlots[a];d.active=!0,d.struck=!1,d.age=0,this.linkA[a].set(t.x,t.y,0,1),this.linkB[a].set(i.x,i.y,this.rand(),0)}scheduleLinks(e,a,t){const i=Math.max(0,a)/60;if(!(i<=0))for(this.linkTimeToNext-=e;this.linkTimeToNext<=0;){this.fireLink(t);const s=Math.max(1e-6,this.rand());this.linkTimeToNext+=-Math.log(s)/i}}ageLinks(e,a){for(let t=0;t<this.linkSlots.length;t++){const i=this.linkSlots[t];if(!i.active)continue;const s=i.age;i.age+=e;const o=i.age;if(this.linkA[t].z=o,!i.struck&&s<z&&o>=z){i.struck=!0;const r=this.linkB[t].x,d=this.linkB[t].y,c=r-Math.floor(r),u=d-Math.floor(d);this.startScan(c,u,a),this.fireKill(c,u),this.kickFlash(C)}o>=it&&(i.active=!1,this.linkA[t].w=0)}}scriptedMassScan(){this.pendingBurst=this.stampSlotCount,this.burstTimer=0,this.startWave(),this.kickFlash(F)}scriptedDrop(){this.kickFlash(F),this.gridSlam=ht,this.startWave()}startWave(){this.waveCx=this.rand(),this.waveCy=this.rand(),this.waveActive=!0,this.waveAge=0}scheduleWaves(e,a){if(this.waveActive)if(this.waveAge+=e,this.waveAge>=P)this.waveActive=!1,this.field.wave.w=0;else{const i=this.sst(this.waveAge/P);this.field.wave.set(this.waveCx,this.waveCy,i*Qe,et*(1-this.waveAge/P))}const t=Math.max(0,a)/60;if(!(t<=0))for(this.waveTimeToNext-=e;this.waveTimeToNext<=0;){this.startWave();const i=Math.max(1e-6,this.rand());this.waveTimeToNext+=-Math.log(i)/t}}fireRunner(){let e=this.runnerSlots.findIndex(o=>!o.active);if(e<0){e=0;let o=this.runnerSlots[0].age;for(let r=1;r<this.runnerSlots.length;r++)this.runnerSlots[r].age>o&&(o=this.runnerSlots[r].age,e=r)}const a=this.cover,t=this.rand()<.5?0:1,i=t===0?(this.rand()-.5)*a.y:(this.rand()-.5)*a.x,s=this.runnerSlots[e];s.active=!0,s.age=0,this.runnerValues[e].set(t,i,0,1)}ageRunners(e){for(let a=0;a<this.runnerSlots.length;a++){const t=this.runnerSlots[a];t.active&&(t.age+=e,t.age>=rt?(t.active=!1,this.runnerValues[a].w=0):this.runnerValues[a].z=t.age)}}scheduleRunners(e,a){const t=Math.max(0,a)/60;if(!(t<=0))for(this.runnerTimeToNext-=e;this.runnerTimeToNext<=0;){this.fireRunner();const i=Math.max(1e-6,this.rand());this.runnerTimeToNext+=-Math.log(i)/t}}fireKill(e,a){let t=this.killSlots.findIndex(s=>!s.active);if(t<0){t=0;let s=this.killSlots[0].age;for(let o=1;o<this.killSlots.length;o++)this.killSlots[o].age>s&&(s=this.killSlots[o].age,t=o)}const i=this.killSlots[t];i.active=!0,i.age=0,this.killValues[t].set(e,a,0,1)}ageKills(e){for(let a=0;a<this.killSlots.length;a++){const t=this.killSlots[a];t.active&&(t.age+=e,t.age>=ot?(t.active=!1,this.killValues[a].w=0):this.killValues[a].z=t.age)}}activatePoke(e,a){let t=this.pokeSlots.findIndex(s=>!s.active);t<0&&(t=0);const i=this.pokeSlots[t];i.active=!0,i.age=0,this.field.pokes[t].set(e,a,Rt,Ct)}updatePokeAges(e){for(let a=0;a<this.pokeSlots.length;a++){const t=this.pokeSlots[a];t.active&&(t.age+=e,t.age>=Tt&&(t.active=!1,this.field.pokes[a].w=0))}}activateRipple(e,a){let t=this.rippleSlots.findIndex(s=>!s.active);t<0&&(t=0);const i=this.rippleSlots[t];i.active=!0,i.age=0,this.rippleValues[t].set(e-Math.floor(e),a-Math.floor(a),0,1)}updateRippleAges(e){for(let a=0;a<this.rippleSlots.length;a++){const t=this.rippleSlots[a];t.active&&(t.age+=e,t.age>=Ue?(t.active=!1,this.rippleValues[a].w=0):this.rippleValues[a].z=t.age)}}warmup(e,a,t){this.field.clearField(),this.field.setActParams(e);const i=this.pinnedClassified??e.classifiedFloor;this.field.seedField(i,e.commFreq,t),this.classified=i;for(let s=0;s<a;s++)this.scheduleScans(T,e.scanRate,e),this.ageScans(T,e),this.field.step(T,1)}update(e,a){const t=De(a.time),i=t.actIndex===3||t.actIndex===4?m[t.actIndex]:t.params;this.lastDt=e;const s=this.pinnedOrder??Ce(a.time).order;this.firstUpdate&&(this.firstUpdate=!1,this.warmup(i,we,s)),this.lastSongTime>=0&&a.time<this.lastSongTime-10&&this.warmup(i,xe,s),this.lastSongTime>=0&&a.time-this.lastSongTime>=0&&a.time-this.lastSongTime<.5&&(this.lastSongTime<W&&a.time>=W&&this.scriptedMassScan(),this.lastSongTime<K&&a.time>=K&&this.scriptedDrop(),this.lastSongTime<q&&a.time>=q&&this.kickFlash(F),this.lastSongTime<X&&a.time>=X&&(this.flickerEnv=1)),this.lastSongTime=a.time;const o=Math.min(1,e*8);this.bassE+=(a.bass-this.bassE)*o,this.midE+=(a.mid-this.midE)*o,this.highE+=(a.high-this.highE)*o;const r=Math.min(1,e*1.5);if(this.bassSlowE+=(a.bass-this.bassSlowE)*r,this.highSlowE+=(a.high-this.highSlowE)*r,this.onsetCooldown-=e,this.onsetCooldown<=0&&this.bassE-this.bassSlowE>Ie&&(i.scanRate>.5&&this.fireScan(i),this.kickFlash(C),this.beatCount++,this.beatFlash=1,this.onsetCount++,this.onsetCount%2===0?t.actIndex===4&&!this.waveActive&&this.startWave():i.linkRate>.5&&this.fireLink(i),this.beatPulse=Math.min(ft,this.beatPulse+dt),this.gridModePending&&this.advanceGridMode(),this.onsetCooldown=Le),this.highOnsetCooldown-=e,this.highOnsetCooldown<=0&&this.highE-this.highSlowE>Oe&&(this.kickSpark(),i.runnerRate>.5&&this.fireRunner(),this.highOnsetCooldown=_e),this.forceSparkAlways&&(this.sparkDebugTimer-=e,this.sparkDebugTimer<=0&&(this.kickSpark(),this.sparkDebugTimer=Be)),this.spark*=Math.exp(-7*e),this.forceScanAlways&&(this.scanDebugTimer-=e,this.scanDebugTimer<=0&&(this.fireScan(i),this.scanDebugTimer=Je)),this.scheduleScans(e,i.scanRate,i),this.scheduleWaves(e,i.waveRate),this.forceWaveAlways&&(this.waveDebugTimer-=e,this.waveDebugTimer<=0&&(this.startWave(),this.waveDebugTimer=3)),this.scheduleLinks(e,i.linkRate,i),this.ageLinks(e,i),this.forceLinkAlways&&(this.linkDebugTimer-=e,this.linkDebugTimer<=0&&(this.fireLink(i),this.linkDebugTimer=at)),this.scheduleRunners(e,i.runnerRate),this.ageRunners(e),this.forceRunAlways&&(this.runDebugTimer-=e,this.runDebugTimer<=0&&(this.fireRunner(),this.runDebugTimer=ct)),this.ageKills(e),this.forceKillAlways&&(this.killDebugTimer-=e,this.killDebugTimer<=0)){const{x:f,y:l}=this.pickVisiblePoint(i);this.fireKill(f-Math.floor(f),l-Math.floor(l)),this.killDebugTimer=lt}if(this.forcePopAlways&&(this.popDebugTimer-=e,this.popDebugTimer<=0&&(this.beatFlash=1,this.beatCount++,this.popDebugTimer=mt)),this.pendingBurst>0)for(this.burstTimer-=e;this.burstTimer<=0&&this.pendingBurst>0;)this.fireScan(i),this.pendingBurst--,this.burstTimer+=.18;this.pinnedClassified!==null?this.classified=this.pinnedClassified:this.classified=Math.max(this.classified,i.classifiedFloor),this.ageScans(e,i),this.updatePokeAges(e),this.updateRippleAges(e),this.classified=Math.min(1,this.classified),this.pinnedClassified!==null&&(this.classified=this.pinnedClassified),this.flash*=Math.exp(-3.4*e),this.gridSlam*=Math.exp(-2.5*e),this.beatPulse*=Math.exp(-4*e),this.beatFlash*=Math.exp(-2.8*e),this.updateFx(e,i),this.pinnedGridMode===null&&(this.gridModeTimer-=e,this.gridModeTimer<=0&&!this.gridModePending&&(this.gridModePending=!0,this.gridModePendingElapsed=0),this.gridModePending&&(this.gridModePendingElapsed+=e,this.gridModePendingElapsed>=pt&&this.advanceGridMode())),this.forceFlickerAlways?this.flickerEnv=1:this.flickerEnv*=Math.exp(-.6*e);const d=this.forceLifeFast?ut:1;this.lifeClock+=e*i.lifeRate*d*(1+this.beatPulse*3),this.field.setVitalityMod(1+this.bassE*.5),this.field.setActParams(i);let c;if(t.actIndex===0){const f=p[1]-p[0],l=t.localT*f,g=this.sst(Math.min(1,l/wt)),I=ee+(m[0].zoom-ee)*g;c=I+(m[1].zoom-I)*t.blend,this.userPanned||this.pan.set(se.x*(1-g),se.y*(1-g))}else if(t.actIndex===2)c=m[2].zoom;else if(t.actIndex===3){const f=p[4]-p[3],l=t.localT*f,g=this.sst(Math.min(1,l/xt));c=m[2].zoom+(m[3].zoom-m[2].zoom)*g}else if(t.actIndex===4){const f=p[5]-p[4],l=t.localT*f;l<te?c=m[3].zoom+(m[4].zoom-m[3].zoom)*this.sst(l/te):c=i.zoom}else c=i.zoom;const u=kt*(1-.7*i.survivorFocus);c*=1+u*Math.sin(a.time*bt+this.breathPhase);const n=this.material.uniforms;n.uTime.value+=e,n.uZoom.value=c,n.uCommFreq.value=i.commFreq,n.uChurn.value=i.churn,n.uChatter.value=i.chatterRate*(1+this.midE*.8),n.uWarmth.value=i.warmth,n.uSparkle.value=this.highE,n.uGlyphKick.value=this.spark,n.uGlyphSeed.value=this.sparkSeed,n.uEnergy.value=Re(a.time).energy,n.uFlash.value=this.flash,n.uClassified.value=this.classified,n.uDesat.value=Math.min(1,this.classified*.6),n.uMachineFrac.value=i.machineFrac,n.uMachineOrder.value=s,n.uGridStrength.value=i.gridStrength,n.uGridFine.value=i.gridFine,n.uGlyphDensity.value=i.glyphDensity,n.uHueSat.value=i.hueSat,n.uRustMix.value=i.rustMix,n.uInkPersist.value=i.inkPersist,n.uVignette.value=i.vignette,n.uMotes.value=i.motes,n.uGroundLight.value=i.groundLight,n.uSurvivorFocus.value=i.survivorFocus,n.uGridSlam.value=this.gridSlam,n.uFlicker.value=this.flickerEnv,n.uLifeClock.value=this.lifeClock,n.uPresence.value=i.presence,n.uWriggle.value=i.wriggle,n.uDrift.value=i.drift,n.uEventVivid.value=i.eventVivid,n.uBeatCount.value=this.beatCount,n.uBeatFlash.value=this.beatFlash,n.uBeatPop.value=i.beatPop,n.uGridLife.value=i.gridLife,n.uGridMode.value=this.pinnedGridMode??this.gridMode,n.uAura.value=this.forceAuraAlways?1:i.aura,n.uAuraPulse.value=this.bassE;const b=this.cover;if(this.held){if(e>1e-5){const f=Math.min(1,e*Mt),l=Math.min(y,Math.max(-y,this.dragDx/e)),g=Math.min(y,Math.max(-y,this.dragDy/e));this.velX+=(l-this.velX)*f,this.velY+=(g-this.velY)*f}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*b.x/c*e,this.pan.y+=this.velY*b.y/c*e;const f=Math.exp(-2.5*e);this.velX*=f,this.velY*=f,Math.abs(this.velX)<oe&&(this.velX=0),Math.abs(this.velY)<oe&&(this.velY=0)}const k=Math.hypot(this.pan.x,this.pan.y);k>D&&(this.pan.x*=D/k,this.pan.y*=D/k,this.velX=0,this.velY=0)}pointer(e){const a=this.material.uniforms.uZoom.value,t=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const i=(e.x-.5)*t.x/a+.5+this.pan.x,s=(e.y-.5)*t.y/a+.5+this.pan.y,o=i-Math.floor(i),r=s-Math.floor(s);this.activatePoke(o,r),this.activateRipple(o,r),this.kickFlash(C);return}if(e.type==="move"){if(!this.held)return;this.userPanned=!0,this.pan.x+=e.dx*t.x/a,this.pan.y+=e.dy*t.y/a,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.field.step(this.lastDt,this.stepsPerFrame),this.material.uniforms.uField.value=this.field.texture,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}resize(e,a){if(!this.material||e<=0||a<=0)return;const t=Math.min(3.5,Math.max(.28,e/a));t>=1?this.cover.set(t,1):this.cover.set(1,1/t),this.material.uniforms.uCover.value.copy(this.cover)}dispose(){this.field.dispose(),this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const Et={default:()=>new Ft},It=Et.default;export{It as default};
//# sourceMappingURL=index-S_sms3_n.js.map
