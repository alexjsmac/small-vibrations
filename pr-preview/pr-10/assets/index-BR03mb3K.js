import{a as L,R as O,L as _,b as oe,H as le,V as p,c as b,S as ee,O as te,d as ae,e as A,M as ie}from"./three-vlqji54k.js";import{m as ne}from"./random-DL1jLgMw.js";const re=512,ce=256,H=2,he=1,V=6,fe=4,C=4,de=240,ue=60,R=1/30,G=256,me=.05,T=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`}class we{uniforms;stamps;pokes;wave;renderer;texSize;targets;readIndex=0;scene;camera;quad;simMaterial;initMaterial;seedMaterial;params=null;vitalityMod=1;constructor(e,i,t){this.renderer=e,this.texSize=i?re:ce;const a={type:le,format:oe,minFilter:_,magFilter:_,wrapS:O,wrapT:O,depthBuffer:!1,stencilBuffer:!1};this.targets=[new L(this.texSize,this.texSize,a),new L(this.texSize,this.texSize,a)],this.stamps=[];for(let l=0;l<t;l++)this.stamps.push(new p(0,0,.05,0));this.pokes=[];for(let l=0;l<C;l++)this.pokes.push(new p(0,0,.035,0));this.wave=new p(.5,.5,0,0),this.uniforms={uPrev:{value:null},uTexel:{value:new b(1/this.texSize,1/this.texSize)},uDt:{value:0},uDiff:{value:me*(this.texSize/G)*(this.texSize/G)},uVitalityTarget:{value:0},uVitalityMod:{value:1},uClassifyPressure:{value:0},uStamps:{value:this.stamps},uPokes:{value:this.pokes},uWave:{value:this.wave}},this.scene=new ee,this.camera=new te(-1,1,1,-1,0,1);const s=new ae(2,2);this.simMaterial=new A({vertexShader:T,fragmentShader:pe(t),uniforms:this.uniforms,depthTest:!1,depthWrite:!1}),this.initMaterial=new A({vertexShader:T,fragmentShader:ve,depthTest:!1,depthWrite:!1});const o={uSeedFloor:{value:0},uSeedVitality:{value:.8},uCommFreq:{value:6},uSeedOrder:{value:0}};this.seedMaterial=new A({vertexShader:T,fragmentShader:ge(this.texSize),uniforms:o,depthTest:!1,depthWrite:!1}),this.quad=new ie(s,this.initMaterial),this.scene.add(this.quad),this.clearField(),this.quad.material=this.simMaterial}setActParams(e){this.params=e}setVitalityMod(e){this.vitalityMod=e}applyParams(e){this.uniforms.uVitalityTarget.value=e.vitalityTarget,this.uniforms.uVitalityMod.value=this.vitalityMod,this.uniforms.uClassifyPressure.value=e.classifyPressure}step(e,i){const t=this.params;if(!t||i<=0)return;this.applyParams(t),this.uniforms.uDt.value=e/i;const a=this.renderer.getRenderTarget();for(let s=0;s<i;s++){const o=this.targets[this.readIndex],l=this.targets[1-this.readIndex];this.uniforms.uPrev.value=o.texture,this.renderer.setRenderTarget(l),this.renderer.render(this.scene,this.camera),this.readIndex=1-this.readIndex}this.renderer.setRenderTarget(a??null)}seedField(e,i,t){const a=this.seedMaterial.uniforms;a.uSeedFloor.value=e,a.uSeedVitality.value=this.params?.vitalityTarget??.8,a.uCommFreq.value=i,a.uSeedOrder.value=t;const s=this.renderer.getRenderTarget(),o=this.quad.material;this.quad.material=this.seedMaterial;for(const l of this.targets)this.renderer.setRenderTarget(l),this.renderer.render(this.scene,this.camera);this.renderer.setRenderTarget(s??null),this.quad.material=o}clearField(){const e=this.renderer.getRenderTarget(),i=this.quad.material;this.quad.material=this.initMaterial;for(const t of this.targets)this.renderer.setRenderTarget(t),this.renderer.render(this.scene,this.camera);this.renderer.setRenderTarget(e??null),this.quad.material=i}get texture(){return this.targets[this.readIndex].texture}dispose(){this.targets[0].dispose(),this.targets[1].dispose(),this.simMaterial.dispose(),this.initMaterial.dispose(),this.seedMaterial.dispose(),this.quad.geometry.dispose()}}const xe=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = vec4(position.xy, 0.0, 1.0);
}
`;function ke(c,e,i,t){const a=Math.max(1,Math.floor(i)),s=2*a+1,o=Math.max(1,Math.floor(t));return`
precision highp float;
varying vec2 vUv;
...
`}const v=[0,56,128,172,232,316,336.998],d=[{name:"thriving-field",vitalityTarget:.95,churn:.9,presence:.8,lifeRate:.1,wriggle:1,drift:1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,eventVivid:0,gridStrength:0,gridFine:0,glyphDensity:.85,chatterRate:1.2,machineFrac:0,classifiedFloor:0,commFreq:6,zoom:1.18,survivorFocus:0,hueSat:1,warmth:.55,rustMix:.05,inkPersist:0,vignette:.3,motes:.15,groundLight:.9},{name:"first-scans",vitalityTarget:.85,churn:.8,presence:.85,lifeRate:.12,wriggle:.8,drift:.8,scanRate:5,scanDrain:.5,misProb:.3,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,eventVivid:.3,gridStrength:.18,gridFine:.25,glyphDensity:.8,chatterRate:1,machineFrac:.08,classifiedFloor:.06,commFreq:6,zoom:1.05,survivorFocus:0,hueSat:.92,warmth:.5,rustMix:.15,inkPersist:.5,vignette:.32,motes:.08,groundLight:.9},{name:"accelerating-catalogue",vitalityTarget:.7,churn:.75,presence:.8,lifeRate:.16,wriggle:.7,drift:.6,scanRate:16,scanDrain:.7,misProb:.15,classifyPressure:0,waveRate:0,linkRate:2,runnerRate:4,eventVivid:.5,gridStrength:.42,gridFine:.5,glyphDensity:.7,chatterRate:.9,machineFrac:.3,classifiedFloor:.28,commFreq:6,zoom:.98,survivorFocus:0,hueSat:.8,warmth:.45,rustMix:.35,inkPersist:.8,vignette:.35,motes:.05,groundLight:.9},{name:"last-unclassified",vitalityTarget:.9,churn:.6,presence:.75,lifeRate:.08,wriggle:1,drift:.7,scanRate:1.5,scanDrain:.4,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,eventVivid:.4,gridStrength:.1,gridFine:.15,glyphDensity:.9,chatterRate:.7,machineFrac:.35,classifiedFloor:.34,commFreq:6,zoom:2.9,survivorFocus:1,hueSat:.85,warmth:.5,rustMix:.3,inkPersist:.5,vignette:.5,motes:.05,groundLight:.9},{name:"total-classification",vitalityTarget:.25,churn:.5,presence:.9,lifeRate:.3,wriggle:.35,drift:.25,scanRate:34,scanDrain:1,misProb:0,classifyPressure:.35,waveRate:10,linkRate:14,runnerRate:22,eventVivid:1,gridStrength:1,gridFine:1,glyphDensity:.55,chatterRate:1.4,machineFrac:.9,classifiedFloor:.9,commFreq:6,zoom:.82,survivorFocus:0,hueSat:.4,warmth:.35,rustMix:.55,inkPersist:1,vignette:.42,motes:.02,groundLight:.9},{name:"residue",vitalityTarget:.1,churn:.25,presence:.22,lifeRate:.04,wriggle:.15,drift:.1,scanRate:0,scanDrain:0,misProb:0,classifyPressure:0,waveRate:0,linkRate:0,runnerRate:0,eventVivid:.2,gridStrength:.06,gridFine:0,glyphDensity:.08,chatterRate:.3,machineFrac:1,classifiedFloor:.93,commFreq:6,zoom:1.15,survivorFocus:0,hueSat:.2,warmth:.3,rustMix:.45,inkPersist:.35,vignette:.45,motes:.5,groundLight:1}],x=[[0,.22],[30,.34],[36,.28],[54,.3],[56,.36],[100,.45],[126,.5],[128,.58],[167.8,.68],[168.3,.78],[171,.74],[174,.42],[200,.38],[228,.5],[231.7,.55],[232.2,.96],[246,1],[300,.82],[315.6,.7],[316.4,.18],[330,.08],[336.998,.04]],N={energy:0};function ye(c){const e=Math.min(Math.max(c,0),x[x.length-1][0]);let i=0;for(;i<x.length-2&&e>=x[i+1][0];)i++;const t=x[i],a=x[i+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,a[0]-t[0])));return N.energy=t[1]+(a[1]-t[1])*s,N}const k=[[0,0],[128,0],[172,.22],[232,.25],[300,.95],[316,1],[336.998,1]],U={order:0};function Se(c){const e=Math.min(Math.max(c,0),k[k.length-1][0]);let i=0;for(;i<k.length-2&&e>=k[i+1][0];)i++;const t=k[i],a=k[i+1],s=Math.min(1,Math.max(0,(e-t[0])/Math.max(.001,a[0]-t[0])));return U.order=t[1]+(a[1]-t[1])*s,U}const be=6;function Ae(c){const e=Math.min(1,Math.max(0,c));return e*e*(3-2*e)}function Ce(c,e,i){if(i<=0)return c;if(i>=1)return e;const t={...c,name:i<.5?c.name:e.name};for(const a of Object.keys(c)){const s=c[a],o=e[a];typeof s=="number"&&typeof o=="number"&&(t[a]=s+(o-s)*i)}return t}function Re(c){const e=v[v.length-1],i=Math.min(Math.max(c,0),e-.001);let t=0;for(;t<d.length-1&&i>=v[t+1];)t++;const a=v[t],s=v[t+1]??e,o=Math.min(1,Math.max(0,(i-a)/Math.max(.001,s-a))),l=t<d.length-1,f=s-i,r=l?Ae(1-Math.min(1,f/be)):0,u=d[t],n=l?d[t+1]:u;return{params:Ce(u,n,r),actIndex:t,localT:o,blend:r}}const Te=.12,Me=.5,Ee=.09,Fe=.22,M=.35,De=.18,E=1.2,Ie=1.6,Pe=.75,Le=1.2,Oe=.5,B=4,_e=1.2,He=2,Ve=1,Ge=4,Ne=2,Ue=.5,F=1.4,Be=2.2,We=.42,ze=.12,qe=.012,Ke=1.2,D=1.6,Xe=.8,Ye=1,$e=3,W=.35,je=1.4,Ze=1.6,Qe=4,Je=.5,et=.7,z=168,q=232,K=316,X=330,tt=.6,at=1,it=1.5,st=6,Y=9.5,ot=40,lt=8,$=.45,nt=.015,rt=.22;function ct(c,e){const i=c*127.1+e*311.7,t=c*269.5+e*183.3,a=Math.sin(i)*43758.5453,s=Math.sin(t)*43758.5453;return[a-Math.floor(a),s-Math.floor(s)]}const[ht,ft]=ct(0,0),j={x:.22+.56*ht,y:.22+.56*ft},Z=6,Q={x:j.x/Z,y:j.y/Z},J=5e-4,dt=10,S=1.5,I=.6,ut=.35,mt=.05,vt=1;class pt{renderer;scene;camera;field;quad;material;rand;forceScanAlways=!1;forceMisAlways=!1;forceWaveAlways=!1;forceFlickerAlways=!1;forceSparkAlways=!1;forceLinkAlways=!1;forceRunAlways=!1;forceLifeFast=!1;pinnedClassified=null;pinnedOrder=null;full=!0;stampSlotCount=V;stepsPerFrame=H;cover=new b(1,1);pan=new b(0,0);userPanned=!1;bassE=0;midE=0;highE=0;bassSlowE=0;highSlowE=0;onsetCooldown=0;highOnsetCooldown=0;flash=0;beatPulse=0;lifeClock=0;spark=0;sparkSeed=0;sparkDebugTimer=0;scanSlots=[];scanA=[];scanB=[];scanTimeToNext=0;scanDebugTimer=0;pendingBurst=0;burstTimer=0;waveActive=!1;waveAge=0;waveCx=.5;waveCy=.5;waveTimeToNext=0;waveDebugTimer=0;rippleSlots=[];rippleValues=[];pokeSlots=[];linkSlots=[];linkA=[];linkB=[];linkTimeToNext=0;linkDebugTimer=0;onsetCount=0;runnerSlots=[];runnerValues=[];runnerTimeToNext=0;runDebugTimer=0;classified=0;gridSlam=0;flickerEnv=0;breathPhase=0;firstUpdate=!0;lastDt=0;lastSongTime=-1;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;init(e){const{renderer:i,seed:t,quality:a}=e;this.renderer=i,this.rand=ne(t^2999220761);const s=new URLSearchParams(location.search),o=s.get("solo"),l=o==="field"?1:o==="patch"?2:o==="glyphs"?3:o==="machine"?4:0;this.forceScanAlways=s.get("scan")==="always",this.forceMisAlways=s.get("mis")==="always",this.forceWaveAlways=s.get("wave")==="always",this.forceFlickerAlways=s.get("flicker")==="always",this.forceSparkAlways=s.get("spark")==="always",this.forceLinkAlways=s.get("link")==="always",this.forceRunAlways=s.get("run")==="always",this.forceLifeFast=s.get("life")==="fast";const f=s.get("classified");if(f!==null){const h=parseFloat(f);Number.isNaN(h)||(this.pinnedClassified=Math.min(1,Math.max(0,h)))}const r=s.get("order");if(r!==null){const h=parseFloat(r);Number.isNaN(h)||(this.pinnedOrder=Math.min(1,Math.max(0,h)))}this.full=a.level==="full",this.stampSlotCount=this.full?V:fe;const u=this.full?He:Ve,n=this.full?Ge:Ne;this.stepsPerFrame=this.full?H:he;for(let h=0;h<this.stampSlotCount;h++)this.scanSlots.push({age:0,active:!1,mislabel:!1}),this.scanA.push(new p(0,0,0,0)),this.scanB.push(new p(0,0,0,0));for(let h=0;h<B;h++)this.rippleSlots.push({age:0,active:!1}),this.rippleValues.push(new p(0,0,0,0));for(let h=0;h<C;h++)this.pokeSlots.push({age:0,active:!1});for(let h=0;h<$e;h++)this.linkSlots.push({age:0,active:!1,struck:!1}),this.linkA.push(new p(0,0,0,0)),this.linkB.push(new p(0,0,0,0));for(let h=0;h<Qe;h++)this.runnerSlots.push({age:0,active:!1}),this.runnerValues.push(new p(0,0,0,0));this.breathPhase=this.rand()*Math.PI*2,this.scene=new ee,this.camera=new te(-1,1,1,-1,0,1),this.field=new we(i,this.full,this.stampSlotCount),this.material=new A({vertexShader:xe,fragmentShader:ke(B,this.stampSlotCount,u,n),depthTest:!1,depthWrite:!1,uniforms:{uField:{value:null},uTime:{value:0},uCover:{value:new b(1,1)},uZoom:{value:1},uPan:{value:this.pan},uCommFreq:{value:6},uEnergy:{value:0},uFlash:{value:0},uSparkle:{value:0},uGlyphKick:{value:0},uGlyphSeed:{value:0},uChurn:{value:0},uChatter:{value:0},uGlyphDensity:{value:0},uMachineFrac:{value:0},uMachineOrder:{value:0},uClassified:{value:0},uGridStrength:{value:0},uGridFine:{value:0},uGridSlam:{value:0},uHueSat:{value:0},uWarmth:{value:0},uRustMix:{value:0},uInkPersist:{value:0},uVignette:{value:0},uMotes:{value:0},uGroundLight:{value:0},uSurvivorFocus:{value:0},uDesat:{value:0},uSoloMode:{value:l},uFlicker:{value:0},uLifeClock:{value:0},uPresence:{value:0},uWriggle:{value:0},uDrift:{value:0},uWaveVis:{value:this.field.wave},uScanA:{value:this.scanA},uScanB:{value:this.scanB},uRipple:{value:this.rippleValues},uEventVivid:{value:0},uLinkA:{value:this.linkA},uLinkB:{value:this.linkB},uRunner:{value:this.runnerValues}}}),this.quad=new ie(new ae(2,2),this.material),this.scene.add(this.quad);const y=i.domElement;this.resize(y.clientWidth||1,y.clientHeight||1)}sst(e){const i=Math.min(1,Math.max(0,e));return i*i*(3-2*i)}kickFlash(e){this.flash=Math.min(Ie,this.flash+e)}kickSpark(){this.spark=Math.min(Le,this.spark+Pe),this.sparkSeed++}startScan(e,i,t){let a=this.scanSlots.findIndex(l=>!l.active);if(a<0){a=0;let l=this.scanSlots[0].age;for(let f=1;f<this.scanSlots.length;f++)this.scanSlots[f].age>l&&(l=this.scanSlots[f].age,a=f)}const s=this.scanSlots[a];s.active=!0,s.age=0;const o=this.forceMisAlways||this.rand()<t.misProb;s.mislabel=o,this.scanA[a].set(e,i,0,1),this.scanB[a].set(We+this.rand()*ze,o?1:0,this.rand(),0)}pickVisiblePoint(e){const i=this.cover,t=this.material.uniforms.uZoom.value;let a=0,s=0;const o=e.survivorFocus>.5;for(let r=0;r<4&&(a=(this.rand()-.5)*.85,s=(this.rand()-.5)*.85,!(!o||Math.hypot(a,s)>.3));r++);const l=.5+this.pan.x+a*i.x/t,f=.5+this.pan.y+s*i.y/t;return{x:l,y:f}}fireScan(e){const{x:i,y:t}=this.pickVisiblePoint(e);this.startScan(i-Math.floor(i),t-Math.floor(t),e)}scheduleScans(e,i,t){const a=Math.max(0,i)/60;if(!(a<=0))for(this.scanTimeToNext-=e;this.scanTimeToNext<=0;){this.fireScan(t),this.kickFlash(De);const s=Math.max(1e-6,this.rand());this.scanTimeToNext+=-Math.log(s)/a}}ageScans(e,i){for(let t=0;t<this.scanSlots.length;t++){const a=this.scanSlots[t];if(!a.active)continue;const s=a.age;a.age+=e;const o=a.age;if(this.scanA[t].z=o,s<F&&o>=F&&(this.classified+=qe),o>=Be)a.active=!1,this.scanA[t].w=0,this.field.stamps[t].w=0;else{const l=this.scanB[t].x,f=o>=Ue&&o<F;this.field.stamps[t].set(this.scanA[t].x,this.scanA[t].y,l/i.commFreq,f?i.scanDrain:0)}}}fireLink(e){let i=this.linkSlots.findIndex(r=>!r.active);if(i<0){i=0;let r=this.linkSlots[0].age;for(let u=1;u<this.linkSlots.length;u++)this.linkSlots[u].age>r&&(r=this.linkSlots[u].age,i=u)}const t=this.pickVisiblePoint(e);let a=this.pickVisiblePoint(e),s=a.x-t.x,o=a.y-t.y;Math.hypot(s,o)>.35&&(a=this.pickVisiblePoint(e),s=a.x-t.x,o=a.y-t.y);const l=Math.hypot(s,o);if(l>.35){const r=.35/l;a={x:t.x+s*r,y:t.y+o*r}}const f=this.linkSlots[i];f.active=!0,f.struck=!1,f.age=0,this.linkA[i].set(t.x,t.y,0,1),this.linkB[i].set(a.x,a.y,this.rand(),0)}scheduleLinks(e,i,t){const a=Math.max(0,i)/60;if(!(a<=0))for(this.linkTimeToNext-=e;this.linkTimeToNext<=0;){this.fireLink(t);const s=Math.max(1e-6,this.rand());this.linkTimeToNext+=-Math.log(s)/a}}ageLinks(e,i){for(let t=0;t<this.linkSlots.length;t++){const a=this.linkSlots[t];if(!a.active)continue;const s=a.age;a.age+=e;const o=a.age;if(this.linkA[t].z=o,!a.struck&&s<W&&o>=W){a.struck=!0;const l=this.linkB[t].x,f=this.linkB[t].y;this.startScan(l-Math.floor(l),f-Math.floor(f),i),this.kickFlash(M)}o>=je&&(a.active=!1,this.linkA[t].w=0)}}scriptedMassScan(){this.pendingBurst=this.stampSlotCount,this.burstTimer=0,this.startWave(),this.kickFlash(E)}scriptedDrop(){this.kickFlash(E),this.gridSlam=tt,this.startWave()}startWave(){this.waveCx=this.rand(),this.waveCy=this.rand(),this.waveActive=!0,this.waveAge=0}scheduleWaves(e,i){if(this.waveActive)if(this.waveAge+=e,this.waveAge>=D)this.waveActive=!1,this.field.wave.w=0;else{const a=this.sst(this.waveAge/D);this.field.wave.set(this.waveCx,this.waveCy,a*Xe,Ye*(1-this.waveAge/D))}const t=Math.max(0,i)/60;if(!(t<=0))for(this.waveTimeToNext-=e;this.waveTimeToNext<=0;){this.startWave();const a=Math.max(1e-6,this.rand());this.waveTimeToNext+=-Math.log(a)/t}}fireRunner(){let e=this.runnerSlots.findIndex(o=>!o.active);if(e<0){e=0;let o=this.runnerSlots[0].age;for(let l=1;l<this.runnerSlots.length;l++)this.runnerSlots[l].age>o&&(o=this.runnerSlots[l].age,e=l)}const i=this.cover,t=this.rand()<.5?0:1,a=t===0?(this.rand()-.5)*i.y:(this.rand()-.5)*i.x,s=this.runnerSlots[e];s.active=!0,s.age=0,this.runnerValues[e].set(t,a,0,1)}ageRunners(e){for(let i=0;i<this.runnerSlots.length;i++){const t=this.runnerSlots[i];t.active&&(t.age+=e,t.age>=Je?(t.active=!1,this.runnerValues[i].w=0):this.runnerValues[i].z=t.age)}}scheduleRunners(e,i){const t=Math.max(0,i)/60;if(!(t<=0))for(this.runnerTimeToNext-=e;this.runnerTimeToNext<=0;){this.fireRunner();const a=Math.max(1e-6,this.rand());this.runnerTimeToNext+=-Math.log(a)/t}}activatePoke(e,i){let t=this.pokeSlots.findIndex(s=>!s.active);t<0&&(t=0);const a=this.pokeSlots[t];a.active=!0,a.age=0,this.field.pokes[t].set(e,i,mt,vt)}updatePokeAges(e){for(let i=0;i<this.pokeSlots.length;i++){const t=this.pokeSlots[i];t.active&&(t.age+=e,t.age>=ut&&(t.active=!1,this.field.pokes[i].w=0))}}activateRipple(e,i){let t=this.rippleSlots.findIndex(s=>!s.active);t<0&&(t=0);const a=this.rippleSlots[t];a.active=!0,a.age=0,this.rippleValues[t].set(e-Math.floor(e),i-Math.floor(i),0,1)}updateRippleAges(e){for(let i=0;i<this.rippleSlots.length;i++){const t=this.rippleSlots[i];t.active&&(t.age+=e,t.age>=_e?(t.active=!1,this.rippleValues[i].w=0):this.rippleValues[i].z=t.age)}}warmup(e,i,t){this.field.clearField(),this.field.setActParams(e);const a=this.pinnedClassified??e.classifiedFloor;this.field.seedField(a,e.commFreq,t),this.classified=a;for(let s=0;s<i;s++)this.scheduleScans(R,e.scanRate,e),this.ageScans(R,e),this.field.step(R,1)}update(e,i){const t=Re(i.time),a=t.actIndex===3||t.actIndex===4?d[t.actIndex]:t.params;this.lastDt=e;const s=this.pinnedOrder??Se(i.time).order;this.firstUpdate&&(this.firstUpdate=!1,this.warmup(a,de,s)),this.lastSongTime>=0&&i.time<this.lastSongTime-10&&this.warmup(a,ue,s),this.lastSongTime>=0&&i.time-this.lastSongTime>=0&&i.time-this.lastSongTime<.5&&(this.lastSongTime<z&&i.time>=z&&this.scriptedMassScan(),this.lastSongTime<q&&i.time>=q&&this.scriptedDrop(),this.lastSongTime<K&&i.time>=K&&this.kickFlash(E),this.lastSongTime<X&&i.time>=X&&(this.flickerEnv=1)),this.lastSongTime=i.time;const o=Math.min(1,e*8);this.bassE+=(i.bass-this.bassE)*o,this.midE+=(i.mid-this.midE)*o,this.highE+=(i.high-this.highE)*o;const l=Math.min(1,e*1.5);if(this.bassSlowE+=(i.bass-this.bassSlowE)*l,this.highSlowE+=(i.high-this.highSlowE)*l,this.onsetCooldown-=e,this.onsetCooldown<=0&&this.bassE-this.bassSlowE>Te&&(a.scanRate>.5&&this.fireScan(a),this.kickFlash(M),this.onsetCount++,this.onsetCount%2===0?t.actIndex===4&&!this.waveActive&&this.startWave():a.linkRate>.5&&this.fireLink(a),this.beatPulse=Math.min(it,this.beatPulse+at),this.onsetCooldown=Me),this.highOnsetCooldown-=e,this.highOnsetCooldown<=0&&this.highE-this.highSlowE>Ee&&(this.kickSpark(),a.runnerRate>.5&&this.fireRunner(),this.highOnsetCooldown=Fe),this.forceSparkAlways&&(this.sparkDebugTimer-=e,this.sparkDebugTimer<=0&&(this.kickSpark(),this.sparkDebugTimer=Oe)),this.spark*=Math.exp(-7*e),this.forceScanAlways&&(this.scanDebugTimer-=e,this.scanDebugTimer<=0&&(this.fireScan(a),this.scanDebugTimer=Ke)),this.scheduleScans(e,a.scanRate,a),this.scheduleWaves(e,a.waveRate),this.forceWaveAlways&&(this.waveDebugTimer-=e,this.waveDebugTimer<=0&&(this.startWave(),this.waveDebugTimer=3)),this.scheduleLinks(e,a.linkRate,a),this.ageLinks(e,a),this.forceLinkAlways&&(this.linkDebugTimer-=e,this.linkDebugTimer<=0&&(this.fireLink(a),this.linkDebugTimer=Ze)),this.scheduleRunners(e,a.runnerRate),this.ageRunners(e),this.forceRunAlways&&(this.runDebugTimer-=e,this.runDebugTimer<=0&&(this.fireRunner(),this.runDebugTimer=et)),this.pendingBurst>0)for(this.burstTimer-=e;this.burstTimer<=0&&this.pendingBurst>0;)this.fireScan(a),this.pendingBurst--,this.burstTimer+=.18;this.pinnedClassified!==null?this.classified=this.pinnedClassified:this.classified=Math.max(this.classified,a.classifiedFloor),this.ageScans(e,a),this.updatePokeAges(e),this.updateRippleAges(e),this.classified=Math.min(1,this.classified),this.pinnedClassified!==null&&(this.classified=this.pinnedClassified),this.flash*=Math.exp(-3.4*e),this.gridSlam*=Math.exp(-2.5*e),this.beatPulse*=Math.exp(-4*e),this.forceFlickerAlways?this.flickerEnv=1:this.flickerEnv*=Math.exp(-.6*e);const f=this.forceLifeFast?st:1;this.lifeClock+=e*a.lifeRate*f*(1+this.beatPulse*3),this.field.setVitalityMod(1+this.bassE*.5),this.field.setActParams(a);let r;if(t.actIndex===0){const m=v[1]-v[0],g=t.localT*m,w=this.sst(Math.min(1,g/ot)),P=Y+(d[0].zoom-Y)*w;r=P+(d[1].zoom-P)*t.blend,this.userPanned||this.pan.set(Q.x*(1-w),Q.y*(1-w))}else if(t.actIndex===2)r=d[2].zoom;else if(t.actIndex===3){const m=v[4]-v[3],g=t.localT*m,w=this.sst(Math.min(1,g/lt));r=d[2].zoom+(d[3].zoom-d[2].zoom)*w}else if(t.actIndex===4){const m=v[5]-v[4],g=t.localT*m;g<$?r=d[3].zoom+(d[4].zoom-d[3].zoom)*this.sst(g/$):r=a.zoom}else r=a.zoom;const u=nt*(1-.7*a.survivorFocus);r*=1+u*Math.sin(i.time*rt+this.breathPhase);const n=this.material.uniforms;n.uTime.value+=e,n.uZoom.value=r,n.uCommFreq.value=a.commFreq,n.uChurn.value=a.churn,n.uChatter.value=a.chatterRate*(1+this.midE*.8),n.uWarmth.value=a.warmth,n.uSparkle.value=this.highE,n.uGlyphKick.value=this.spark,n.uGlyphSeed.value=this.sparkSeed,n.uEnergy.value=ye(i.time).energy,n.uFlash.value=this.flash,n.uClassified.value=this.classified,n.uDesat.value=Math.min(1,this.classified*.6),n.uMachineFrac.value=a.machineFrac,n.uMachineOrder.value=s,n.uGridStrength.value=a.gridStrength,n.uGridFine.value=a.gridFine,n.uGlyphDensity.value=a.glyphDensity,n.uHueSat.value=a.hueSat,n.uRustMix.value=a.rustMix,n.uInkPersist.value=a.inkPersist,n.uVignette.value=a.vignette,n.uMotes.value=a.motes,n.uGroundLight.value=a.groundLight,n.uSurvivorFocus.value=a.survivorFocus,n.uGridSlam.value=this.gridSlam,n.uFlicker.value=this.flickerEnv,n.uLifeClock.value=this.lifeClock,n.uPresence.value=a.presence,n.uWriggle.value=a.wriggle,n.uDrift.value=a.drift,n.uEventVivid.value=a.eventVivid;const y=this.cover;if(this.held){if(e>1e-5){const m=Math.min(1,e*dt),g=Math.min(S,Math.max(-S,this.dragDx/e)),w=Math.min(S,Math.max(-S,this.dragDy/e));this.velX+=(g-this.velX)*m,this.velY+=(w-this.velY)*m}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*y.x/r*e,this.pan.y+=this.velY*y.y/r*e;const m=Math.exp(-2.5*e);this.velX*=m,this.velY*=m,Math.abs(this.velX)<J&&(this.velX=0),Math.abs(this.velY)<J&&(this.velY=0)}const h=Math.hypot(this.pan.x,this.pan.y);h>I&&(this.pan.x*=I/h,this.pan.y*=I/h,this.velX=0,this.velY=0)}pointer(e){const i=this.material.uniforms.uZoom.value,t=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const a=(e.x-.5)*t.x/i+.5+this.pan.x,s=(e.y-.5)*t.y/i+.5+this.pan.y,o=a-Math.floor(a),l=s-Math.floor(s);this.activatePoke(o,l),this.activateRipple(o,l),this.kickFlash(M);return}if(e.type==="move"){if(!this.held)return;this.userPanned=!0,this.pan.x+=e.dx*t.x/i,this.pan.y+=e.dy*t.y/i,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.field.step(this.lastDt,this.stepsPerFrame),this.material.uniforms.uField.value=this.field.texture,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}resize(e,i){if(!this.material||e<=0||i<=0)return;const t=Math.min(3.5,Math.max(.28,e/i));t>=1?this.cover.set(t,1):this.cover.set(1,1/t),this.material.uniforms.uCover.value.copy(this.cover)}dispose(){this.field.dispose(),this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const gt={default:()=>new pt},kt=gt.default;export{kt as default};
//# sourceMappingURL=index-BR03mb3K.js.map
