import{V as $,c as g,S as ee,O as te,e as ae,M as se,d as oe}from"./three-vlqji54k.js";import{m as ie}from"./random-DL1jLgMw.js";const ne=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`}const m=[0,12,78,104,140,162,184,200.042],S=[{name:"breath",breathAmp:1,clumpPresence:.85,lifeRate:.06,motionSpeed:.5,heat:.7,paleness:0,ghostAmp:.25,frontNoise:0,scrubGlow:0,glintAmp:0,crackleRate:0,strikeRate:0,strikeHeal:.22,strikeSize:.05,bloomRate:10,bloomAmp:1,watermark:0,sterileSpec:.2,grade:.05,vignette:.15,driftSpeed:.4,shake:0},{name:"the-front-appears",breathAmp:.8,clumpPresence:.9,lifeRate:.1,motionSpeed:.7,heat:.8,paleness:0,ghostAmp:.35,frontNoise:.5,scrubGlow:.5,glintAmp:.4,crackleRate:4,strikeRate:9,strikeHeal:.2,strikeSize:.07,bloomRate:0,bloomAmp:.5,watermark:.25,sterileSpec:.35,grade:.1,vignette:.2,driftSpeed:.5,shake:0},{name:"losing-ground",breathAmp:.7,clumpPresence:.9,lifeRate:.14,motionSpeed:.9,heat:1,paleness:.05,ghostAmp:.3,frontNoise:.65,scrubGlow:.65,glintAmp:.55,crackleRate:8,strikeRate:14,strikeHeal:.12,strikeSize:.08,bloomRate:0,bloomAmp:.3,watermark:.35,sterileSpec:.4,grade:.15,vignette:.25,driftSpeed:.6,shake:0},{name:"remission",breathAmp:.55,clumpPresence:.75,lifeRate:.05,motionSpeed:.4,heat:.45,paleness:.55,ghostAmp:.3,frontNoise:.45,scrubGlow:.4,glintAmp:.35,crackleRate:2,strikeRate:2,strikeHeal:.06,strikeSize:.06,bloomRate:4,bloomAmp:.7,watermark:.3,sterileSpec:.35,grade:.12,vignette:.3,driftSpeed:0,shake:0},{name:"the-purge",breathAmp:.5,clumpPresence:.8,lifeRate:.3,motionSpeed:1.2,heat:.9,paleness:.15,ghostAmp:.15,frontNoise:1,scrubGlow:1,glintAmp:1,crackleRate:18,strikeRate:22,strikeHeal:0,strikeSize:.11,bloomRate:0,bloomAmp:.2,watermark:.45,sterileSpec:.5,grade:.2,vignette:.35,driftSpeed:.8,shake:1},{name:"sterile",breathAmp:.25,clumpPresence:.5,lifeRate:.06,motionSpeed:.3,heat:.6,paleness:.3,ghostAmp:0,frontNoise:.6,scrubGlow:.7,glintAmp:.5,crackleRate:3,strikeRate:3,strikeHeal:0,strikeSize:.08,bloomRate:0,bloomAmp:.3,watermark:.7,sterileSpec:.8,grade:.5,vignette:.25,driftSpeed:0,shake:0},{name:"last-breath",breathAmp:.15,clumpPresence:.25,lifeRate:.02,motionSpeed:.1,heat:.5,paleness:.4,ghostAmp:0,frontNoise:.3,scrubGlow:.2,glintAmp:0,crackleRate:0,strikeRate:0,strikeHeal:0,strikeSize:.05,bloomRate:1.5,bloomAmp:.9,watermark:.3,sterileSpec:1,grade:.8,vignette:.4,driftSpeed:0,shake:0}],le=6;function ce(i){const e=Math.min(1,Math.max(0,i));return e*e*(3-2*e)}function he(i,e,s){if(s<=0)return i;if(s>=1)return e;const a={...i,name:s<.5?i.name:e.name};for(const t of Object.keys(i)){const n=i[t],o=e[t];typeof n=="number"&&typeof o=="number"&&(a[t]=n+(o-n)*s)}return a}function B(i,e){const s=Math.min(Math.max(e,0),i[i.length-1][0]);let a=0;for(;a<i.length-2&&s>=i[a+1][0];)a++;const t=i[a],n=i[a+1],o=Math.min(1,Math.max(0,(s-t[0])/Math.max(.001,n[0]-t[0])));return t[1]+(n[1]-t[1])*o}function de(i){const e=m[m.length-1],s=Math.min(Math.max(i,0),e-.001);let a=0;for(;a<S.length-1&&s>=m[a+1];)a++;const t=m[a],n=m[a+1]??e,o=Math.min(1,Math.max(0,(s-t)/Math.max(.001,n-t))),r=a<S.length-1,h=n-s,u=r?ce(1-Math.min(1,h/le)):0,f=S[a],d=r?S[a+1]:f;return{params:he(f,d,u),actIndex:a,localT:o,blend:u}}const fe=[[0,0],[12,.03],[30,.12],[78,.28],[104,.44],[110,.41],[118,.34],[128,.33],[139.7,.33],[140.3,.52],[143.8,.56],[144.4,.66],[150,.72],[162,.86],[167.8,.87],[168.3,.905],[173.7,.91],[174.3,.945],[179.7,.95],[180.3,.975],[183.7,.98],[184.3,1],[200.042,1]];function ue(i){return B(fe,i)}const me=[[0,1.45],[12,1.45],[40,1.25],[78,1.15],[104,1.22],[139.7,1.22],[140.3,1.08],[162,1.02],[164,1],[178,.3],[184,.28],[200.042,.28]];function G(i){return B(me,i)}const pe=[[0,.25],[12,.4],[40,.5],[78,.65],[104,.55],[105.5,.3],[108,.35],[126,.4],[139.7,.45],[140.3,.9],[144,1],[148,.95],[161.7,.9],[162.3,.55],[168,.45],[174,.35],[180,.25],[184,.12],[196,.02],[200.042,0]];function ge(i){return B(pe,i)}const be=(()=>{const i=[0];for(let e=0;e<S.length;e++){const s=m[e+1]-m[e];i.push(i[e]+S[e].lifeRate*s)}return i})();function ve(i){const e=m[m.length-1],s=Math.min(Math.max(i,0),e);let a=0;for(;a<S.length-1&&s>=m[a+1];)a++;return be[a]+S[a].lifeRate*(s-m[a])}class L{slots;constructor(e){this.slots=[];for(let s=0;s<e;s++)this.slots.push(new $(0,0,0,0))}fire(){let e=this.slots.findIndex(a=>a.w<=0);if(e<0){e=0;let a=this.slots[0].z;for(let t=1;t<this.slots.length;t++)this.slots[t].z>a&&(a=this.slots[t].z,e=t)}const s=this.slots[e];return s.z=0,s.w=1,e}age(e,s){for(let a=0;a<this.slots.length;a++){const t=this.slots[a];t.w<=0||(t.z+=e,s>0&&t.z>=s&&(t.w=0))}}clearAll(){for(let e=0;e<this.slots.length;e++)this.slots[e].w=0}}function F(i,e){if(e<=0)return 1/0;const s=e/60;return-Math.log(1-i())/s}const Se=12,we=1.5,ke=.18,Ee=.035,H=.008,ye=8,Re=8,_e=.25,Ae=.22,xe=.06,Te=.28,Le=1.02,Oe=1+J*1*.5,Me=.3,Ne=8,U=14,K=1.5,Ce=10,Pe=2.5,W=.45,Ie=1.2,Fe=30,De=.6,Be=.3,Ge=4,He=3,Ue=Be+Ge+He,Ke=.6,We=1.2,ze=10,O=1.5,z=5e-4,w=.35,Xe=6,X=.0035,Ye=.05,qe=.012,je=.8,Y=.006,Ve=4,q=140,j=144,Ze=[168,174,180],V=184,M=.5,Qe=1,$e=.7,Je=.45,et=.6,tt=1.6,at=.5,st=.25,ot=.16,it=.05,nt=.15,rt=.5,D=188,N=191,C=194,Z=199;function Q(i){const e=Math.min(1,Math.max(0,i));return e*e*(3-2*e)}function lt(i){if(i<D)return{phase:0,t:0,seedFade:1};if(i<N){const e=(i-D)/(N-D);return{phase:1,t:Q(e),seedFade:1}}if(i<C){const e=(i-N)/(C-N);return{phase:2,t:Q(e),seedFade:1}}if(i<Z){const e=(i-C)/(Z-C);return{phase:3,t:e,seedFade:1-e}}return{phase:0,t:0,seedFade:0}}class ct{renderer;scene;camera;quad;material;rand;full=!0;cover=new g(1,1);canvasPxW=1;canvasPxH=1;section=null;forceLifeFast=!1;pinnedBreath=null;pinnedSterile=null;pinnedGlint=null;seedOff=new g;beatBonus=0;bassSlow=0;highFast=0;breathPhase=0;motionPhase=0;soloMode=0;strikePool;strikeB=[];forceStrikeAlways=!1;healOverride=null;forceGhostAlways=!1;bassFast=0;bassOnsetEma=0;onsetCooldown=0;tick=0;strikeTimeToNext=0;strikeScheduleRate=0;lastSongTime=0;rMaxEff=0;bloomPool;bloomCooldown=0;bloomTimeToNext=0;forceBloomAlways=!1;pokePool;ripplePool;pan=new g(0,0);panDisplay=new g(0,0);driftAngle=0;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;shake=0;flash=0;crackle=0;crackleSeed=0;highOnsetEma=0;crackleCooldown=0;crackleTimeToNext=0;crackleScheduleRate=0;forceCrackleAlways=!1;crackleDebugTimer=0;forceSeedPointAlways=!1;crackMult=1;init(e){const{renderer:s,seed:a,quality:t}=e;this.renderer=s,this.rand=ie(a^3009248801),this.full=t.level==="full";const n=new URLSearchParams(location.search),o=n.get("solo");let r=0;if(o==="biomass")r=1;else if(o==="front")r=2;else if(o==="events")r=3;else if(o==="ghosts")r=4;else if(o==="ground")r=6;else{const l=o!==null?parseInt(o,10):NaN;r=Number.isFinite(l)?l:0}this.soloMode=r,this.forceLifeFast=n.get("life")==="fast",this.forceStrikeAlways=n.get("strike")==="always",this.forceBloomAlways=n.get("bloom")==="always",this.forceGhostAlways=n.get("ghost")==="always",this.forceCrackleAlways=n.get("crackle")==="always",this.forceSeedPointAlways=n.get("seedpt")==="always";const h=n.get("heal");h==="off"?this.healOverride=0:h==="on"&&(this.healOverride=.22);const u=n.get("breath");if(u!==null){const l=parseFloat(u);Number.isNaN(l)||(this.pinnedBreath=Math.min(1,Math.max(0,l)))}const f=n.get("sterile");if(f!==null){const l=parseFloat(f);Number.isNaN(l)||(this.pinnedSterile=Math.min(1,Math.max(0,l)))}const d=n.get("glint");if(d!==null){const l=parseFloat(d);Number.isNaN(l)||(this.pinnedGlint=Math.min(1,Math.max(0,l)))}const p=n.get("crack");if(p!==null){const l=parseFloat(p);Number.isNaN(l)||(this.crackMult=Math.min(6,Math.max(.25,l)))}const b=()=>(this.rand()<.5?-1:1)*(.1+this.rand()*.14),v=new g(.5+b(),.5+b()),E=this.rand()*.55,y=this.rand()*.35,k=this.rand()<.5?new g(1+E,1-y):new g(1-y,1+E);this.seedOff.set(this.rand()*10,this.rand()*10),this.driftAngle=this.rand()*Math.PI*2;const A=new g(.5+(this.rand()-.5)*.12,.5+(this.rand()-.5)*.12),R=this.full?8:6;this.strikePool=new L(R);for(let l=0;l<R;l++)this.strikeB.push(new $(0,0,0,0));const _=3,x=4,T=4;this.bloomPool=new L(_),this.pokePool=new L(x),this.ripplePool=new L(T),this.scene=new ee,this.camera=new te(-1,1,1,-1,0,1),this.material=new ae({vertexShader:ne,fragmentShader:re({strikeSlots:R,bloomSlots:_,pokeSlots:x,rippleSlots:T,lobes:this.full?5:3,fbmOct:this.full?3:2,grain:this.full,fissures:this.full?4:3}),depthTest:!1,depthWrite:!1,uniforms:{uCover:{value:new g(1,1)},uZoom:{value:1},uFieldPxScale:{value:0},uPan:{value:this.panDisplay},uTime:{value:0},uSterile:{value:0},uSoloMode:{value:r},uLifeClock:{value:0},uPresence:{value:0},uBreath:{value:0},uBreathPhase:{value:0},uMotionPhase:{value:0},uGroundPhase:{value:0},uHeat:{value:0},uPaleness:{value:0},uPocket:{value:v},uStretch:{value:k},uFrontDrift:{value:new g(0,0)},uFrontNoise:{value:0},uRMax:{value:0},uScrubGlow:{value:0},uGlint:{value:0},uWatermark:{value:0},uSterileSpec:{value:0},uTick:{value:0},uStrikeA:{value:this.strikePool.slots},uStrikeB:{value:this.strikeB},uBloom:{value:this.bloomPool.slots},uBloomAmp:{value:0},uPoke:{value:this.pokePool.slots},uRipple:{value:this.ripplePool.slots},uGhostAmp:{value:0},uGhostForce:{value:this.forceGhostAlways?1:0},uEnergy:{value:0},uGrade:{value:0},uVignette:{value:0},uFlash:{value:0},uCrackle:{value:0},uCrackleSeed:{value:0},uFinalPhase:{value:0},uFinalT:{value:0},uSeedFade:{value:0},uFinalPos:{value:A}}}),this.quad=new se(new oe(2,2),this.material),this.scene.add(this.quad);const c=s.domElement;this.resize(c.clientWidth||1,c.clientHeight||1)}update(e,s){const a=s.time,t=this.material.uniforms;t.uTime.value+=e,t.uSterile.value=this.soloMode===3?0:this.pinnedSterile!==null?this.pinnedSterile:ue(a),t.uZoom.value=G(a);const n=de(a),o=n.actIndex===3||n.actIndex===4?S[n.actIndex]:n.params;this.section=n;const r=ge(a);t.uEnergy.value=r,t.uGrade.value=o.grade,t.uVignette.value=o.vignette,Math.abs(a-this.lastSongTime)>Ce&&(this.strikePool.clearAll(),this.strikeTimeToNext=0,this.strikeScheduleRate=0,this.bloomPool.clearAll(),this.bloomTimeToNext=0,this.pokePool.clearAll(),this.ripplePool.clearAll(),this.crackleTimeToNext=0,this.crackleScheduleRate=0),this.lastSongTime<q&&a>=q&&a-this.lastSongTime<M&&(this.flash=Math.max(this.flash,Qe)),this.lastSongTime<j&&a>=j&&a-this.lastSongTime<M&&(this.fireStrike(o,tt,at,!0),this.flash=Math.max(this.flash,$e));for(const c of Ze)this.lastSongTime<c&&a>=c&&a-this.lastSongTime<M&&(this.flash=Math.max(this.flash,Je));this.lastSongTime<V&&a>=V&&a-this.lastSongTime<M&&(this.flash=Math.max(this.flash,et)),this.lastSongTime=a;const h=this.forceSeedPointAlways?{phase:3,t:0,seedFade:1}:lt(a);t.uFinalPhase.value=h.phase,t.uFinalT.value=h.t,t.uSeedFade.value=h.seedFade;const u=this.forceLifeFast?Se:1;t.uLifeClock.value=ve(a)*u+this.beatBonus,t.uPresence.value=o.clumpPresence;const f=this.bassSlow;this.bassSlow+=(s.bass-this.bassSlow)*Math.min(1,e*we),t.uBreath.value=this.pinnedBreath!==null?this.pinnedBreath:this.bassSlow*o.breathAmp,t.uZoom.value=t.uZoom.value*(1-qe*t.uBreath.value);{const c=t.uZoom.value,l=Math.max(this.cover.x,this.cover.y),I=Math.max(1,Math.min(this.canvasPxW,this.canvasPxH));t.uFieldPxScale.value=l/(Math.max(1e-4,c)*I)}this.breathPhase+=e*ke,t.uBreathPhase.value=this.breathPhase,this.motionPhase+=e*o.motionSpeed*(.5+s.mid),t.uMotionPhase.value=this.motionPhase,t.uGroundPhase.value=a*Ee,t.uHeat.value=o.heat,t.uPaleness.value=o.paleness,t.uGhostAmp.value=this.forceGhostAlways?1:o.ghostAmp;const d=t.uFrontDrift.value;d.x+=e*H,d.y+=e*H*.83,t.uFrontNoise.value=o.frontNoise,t.uScrubGlow.value=o.scrubGlow*(.6+.8*this.bassSlow);const p=this.highFast;this.highFast+=(s.high-this.highFast)*Math.min(1,e*ye),t.uGlint.value=this.pinnedGlint!==null?this.pinnedGlint:o.glintAmp*this.highFast,this.highOnsetEma+=(this.highFast-this.highOnsetEma)*Math.min(1,e*st),this.crackleCooldown=Math.max(0,this.crackleCooldown-e);const b=o.crackleRate>0?60/o.crackleRate:1/0,v=Math.max(nt,b);if(o.crackleRate>0&&this.crackleCooldown<=0&&this.highFast>p&&this.highFast>it&&this.highFast>this.highOnsetEma*(1+ot)&&(this.crackleCooldown=v,this.fireCrackle()),this.forceCrackleAlways)this.crackleDebugTimer-=e,this.crackleDebugTimer<=0&&(this.fireCrackle(),this.crackleDebugTimer=rt);else{const c=o.crackleRate*(.5+r);if(c>0)for(this.crackleScheduleRate>0&&this.crackleTimeToNext>0&&(this.crackleTimeToNext*=this.crackleScheduleRate/c),this.crackleScheduleRate=c,this.crackleTimeToNext-=e;this.crackleTimeToNext<=0;)this.fireCrackle(),this.crackleTimeToNext+=F(this.rand,c),this.crackleScheduleRate=c;else this.crackleScheduleRate=0}this.crackle*=Math.exp(-7*e),t.uCrackle.value=this.crackle,t.uCrackleSeed.value=this.crackleSeed,t.uWatermark.value=o.watermark,t.uSterileSpec.value=o.sterileSpec;const E=this.bassFast;this.bassFast+=(s.bass-this.bassFast)*Math.min(1,e*Re),this.bassOnsetEma+=(this.bassFast-this.bassOnsetEma)*Math.min(1,e*_e),this.onsetCooldown=Math.max(0,this.onsetCooldown-e);const y=o.strikeRate>0?60/o.strikeRate:1/0,P=Math.max(Te,y);o.strikeRate>0&&this.onsetCooldown<=0&&this.bassFast>E&&this.bassFast>xe&&this.bassFast>this.bassOnsetEma*(1+Ae)&&(this.onsetCooldown=P,this.tick=1,this.fireStrike(o)),this.tick*=Math.exp(-7*e),t.uTick.value=this.tick,this.flash*=Math.exp(-3.4*e),t.uFlash.value=this.flash;const k=this.forceStrikeAlways?60:o.strikeRate;if(k>0)for(this.strikeScheduleRate>0&&this.strikeTimeToNext>0&&(this.strikeTimeToNext*=this.strikeScheduleRate/k),this.strikeScheduleRate=k,this.strikeTimeToNext-=e;this.strikeTimeToNext<=0;)this.fireStrike(o),this.strikeTimeToNext+=F(this.rand,k),this.strikeScheduleRate=k;else this.strikeScheduleRate=0;this.ageStrikes(e),this.bloomCooldown=Math.max(0,this.bloomCooldown-e),this.bloomCooldown<=0&&f<W&&this.bassSlow>=W&&(this.bloomCooldown=Ie,this.fireBloom());const A=this.forceBloomAlways?Fe:o.bloomRate;if(A>0)for(this.bloomTimeToNext-=e;this.bloomTimeToNext<=0;)this.fireBloom(),this.bloomTimeToNext+=F(this.rand,A);t.uBloomAmp.value=o.bloomAmp,this.bloomPool.age(e,Pe),this.pokePool.age(e,Ue),this.ripplePool.age(e,We);const R=t.uZoom.value;if(this.held){if(e>1e-5){const c=Math.min(1,e*ze),l=Math.min(O,Math.max(-O,this.dragDx/e)),I=Math.min(O,Math.max(-O,this.dragDy/e));this.velX+=(l-this.velX)*c,this.velY+=(I-this.velY)*c}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*this.cover.x/R*e,this.pan.y+=this.velY*this.cover.y/R*e;const c=Math.exp(-2.5*e);this.velX*=c,this.velY*=c,Math.abs(this.velX)<z&&(this.velX=0),Math.abs(this.velY)<z&&(this.velY=0)}else{this.driftAngle+=e*Ye;const c=Math.cos(this.driftAngle),l=Math.sin(this.driftAngle);this.pan.x+=c*o.driftSpeed*X*e,this.pan.y+=l*o.driftSpeed*X*e}this.pan.x>w?(this.pan.x=w,this.velX=0):this.pan.x<-w&&(this.pan.x=-w,this.velX=0),this.pan.y>w?(this.pan.y=w,this.velY=0):this.pan.y<-w&&(this.pan.y=-w,this.velY=0),this.shake*=Math.exp(-6*e);const _=t.uTime.value,x=Math.sin(_*41)*this.shake*Y,T=Math.sin(_*53+1.3)*this.shake*Y;this.panDisplay.set(this.pan.x+x,this.pan.y+T)}pickLivingPoint(e=1){const s=this.material.uniforms,a=s.uCover.value,t=s.uZoom.value,n=this.pan,o=s.uPocket.value,r=s.uStretch.value,h=s.uSterile.value,u=this.rMaxEff*(1-h);for(let f=0;f<Ne;f++){let d=this.rand(),p=this.rand();d=.5+(d-.5)*e,p=.5+(p-.5)*e;const b=(d-.5)*a.x/t+.5+n.x,v=(p-.5)*a.y/t+.5+n.y,E=(b-o.x)*r.x,y=(v-o.y)*r.y;if(Math.hypot(E,y)-u<-.06)return{x:b,y:v}}return null}fireStrike(e,s=1,a=1,t=!1){const n=this.pickLivingPoint(a);if(!n)return;const o=this.strikePool.fire(),r=this.strikePool.slots[o];r.x=n.x,r.y=n.y,s*=this.crackMult;const h=e.strikeSize*s*(.8+this.rand()*.5),u=1.15+this.rand()*.5,f=this.rand()*Math.PI*2,d=t?0:this.healOverride!==null?this.healOverride:e.strikeHeal;this.strikeB[o].set(h,u,f,d),this.section&&this.section.actIndex===Ve&&(this.shake=e.shake*je)}fireBloom(){const e=this.pickLivingPoint(De);if(!e)return;const s=this.bloomPool.fire(),a=this.bloomPool.slots[s];a.x=e.x,a.y=e.y}fireBloomAt(e,s){const a=this.bloomPool.fire(),t=this.bloomPool.slots[a];t.x=e,t.y=s}firePoke(e,s){const a=this.pokePool.fire(),t=this.pokePool.slots[a];t.x=e,t.y=s}fireRipple(e,s){const a=this.ripplePool.fire(),t=this.ripplePool.slots[a];t.x=e,t.y=s}fireCrackle(){this.crackle=1,this.crackleSeed++}ageStrikes(e){this.strikePool.age(e,0);const s=this.strikePool.slots;for(let a=0;a<s.length;a++){const t=s[a];if(t.w<=0)continue;const n=this.strikeB[a].w,o=t.z;if(n>0){const r=Me+1/n;o>=r&&(t.w=0)}else{const r=U-K;o>=U?t.w=0:o>=r&&(t.w=Math.max(0,1-(o-r)/K))}}}resize(e,s){if(!this.material||e<=0||s<=0)return;const a=Math.min(3.5,Math.max(.28,e/s));a>=1?this.cover.set(a,1):this.cover.set(1,1/a),this.material.uniforms.uCover.value.copy(this.cover),this.canvasPxW=this.renderer.domElement.width||1,this.canvasPxH=this.renderer.domElement.height||1,this.rMaxEff=this.computeRMaxEff(),this.material.uniforms.uRMax.value=this.rMaxEff}computeRMaxEff(){const e=this.material.uniforms,s=e.uPocket.value,a=e.uStretch.value,t=G(0),n=.5*this.cover.x/t,o=.5*this.cover.y/t;let r=0;for(const h of[-1,1])for(const u of[-1,1]){const f=.5+h*n,d=.5+u*o,p=(f-s.x)*a.x,b=(d-s.y)*a.y,v=Math.hypot(p,b);v>r&&(r=v)}return r*Oe*Le}pointer(e){const s=this.material.uniforms.uZoom.value,a=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const t=(e.x-.5)*a.x/s+.5+this.pan.x,n=(e.y-.5)*a.y/s+.5+this.pan.y;(this.section?this.section.actIndex:0)===Xe?this.fireBloomAt(t,n):(this.firePoke(t,n),this.tick=Math.max(this.tick,Ke)),this.fireRipple(t,n);return}if(e.type==="move"){if(!this.held)return;this.pan.x+=e.dx*a.x/s,this.pan.y+=e.dy*a.y/s,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}dispose(){this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const ht={default:()=>new ct},ut=ht.default;export{ut as default};
//# sourceMappingURL=index-BJxW3sv3.js.map
