import{V as Q,c as v,S as J,O as ee,e as te,M as ae,d as se}from"./three-vlqji54k.js";import{m as oe}from"./random-DL1jLgMw.js";const ie=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = vec4(position.xy, 0.0, 1.0);
}
`,$=.6;function ne(i){const e=Math.max(1,Math.floor(i.strikeSlots)),s=Math.max(1,Math.floor(i.bloomSlots)),a=Math.max(1,Math.floor(i.pokeSlots)),t=Math.max(1,Math.floor(i.rippleSlots)),n=Math.max(1,Math.floor(i.lobes)),o=Math.max(1,Math.floor(i.fbmOct)),r=Math.max(1,Math.floor(i.fissures)),h=i.grain?`
...
`}const p=[0,12,78,104,140,162,184,200.042],S=[{name:"breath",breathAmp:1,clumpPresence:.85,lifeRate:.06,motionSpeed:.5,heat:.7,paleness:0,ghostAmp:.25,frontNoise:0,scrubGlow:0,glintAmp:0,crackleRate:0,strikeRate:0,strikeHeal:.22,strikeSize:.05,bloomRate:10,bloomAmp:1,watermark:0,sterileSpec:.2,grade:.05,vignette:.15,driftSpeed:.4,shake:0},{name:"the-front-appears",breathAmp:.8,clumpPresence:.9,lifeRate:.1,motionSpeed:.7,heat:.8,paleness:0,ghostAmp:.35,frontNoise:.5,scrubGlow:.5,glintAmp:.4,crackleRate:4,strikeRate:5,strikeHeal:.2,strikeSize:.07,bloomRate:0,bloomAmp:.5,watermark:.25,sterileSpec:.35,grade:.1,vignette:.2,driftSpeed:.5,shake:0},{name:"losing-ground",breathAmp:.7,clumpPresence:.9,lifeRate:.14,motionSpeed:.9,heat:1,paleness:.05,ghostAmp:.3,frontNoise:.65,scrubGlow:.65,glintAmp:.55,crackleRate:8,strikeRate:10,strikeHeal:.12,strikeSize:.08,bloomRate:0,bloomAmp:.3,watermark:.35,sterileSpec:.4,grade:.15,vignette:.25,driftSpeed:.6,shake:0},{name:"remission",breathAmp:.55,clumpPresence:.75,lifeRate:.05,motionSpeed:.4,heat:.45,paleness:.55,ghostAmp:.3,frontNoise:.45,scrubGlow:.4,glintAmp:.35,crackleRate:2,strikeRate:2,strikeHeal:.06,strikeSize:.06,bloomRate:4,bloomAmp:.7,watermark:.3,sterileSpec:.35,grade:.12,vignette:.3,driftSpeed:0,shake:0},{name:"the-purge",breathAmp:.5,clumpPresence:.8,lifeRate:.3,motionSpeed:1.2,heat:.9,paleness:.15,ghostAmp:.15,frontNoise:1,scrubGlow:1,glintAmp:1,crackleRate:18,strikeRate:22,strikeHeal:0,strikeSize:.11,bloomRate:0,bloomAmp:.2,watermark:.45,sterileSpec:.5,grade:.2,vignette:.35,driftSpeed:.8,shake:1},{name:"sterile",breathAmp:.25,clumpPresence:.5,lifeRate:.06,motionSpeed:.3,heat:.6,paleness:.3,ghostAmp:0,frontNoise:.6,scrubGlow:.7,glintAmp:.5,crackleRate:3,strikeRate:3,strikeHeal:0,strikeSize:.08,bloomRate:0,bloomAmp:.3,watermark:.7,sterileSpec:.8,grade:.5,vignette:.25,driftSpeed:0,shake:0},{name:"last-breath",breathAmp:.15,clumpPresence:.25,lifeRate:.02,motionSpeed:.1,heat:.5,paleness:.4,ghostAmp:0,frontNoise:.3,scrubGlow:.2,glintAmp:0,crackleRate:0,strikeRate:0,strikeHeal:0,strikeSize:.05,bloomRate:1.5,bloomAmp:.9,watermark:.3,sterileSpec:1,grade:.8,vignette:.4,driftSpeed:0,shake:0}],re=6;function le(i){const e=Math.min(1,Math.max(0,i));return e*e*(3-2*e)}function ce(i,e,s){if(s<=0)return i;if(s>=1)return e;const a={...i,name:s<.5?i.name:e.name};for(const t of Object.keys(i)){const n=i[t],o=e[t];typeof n=="number"&&typeof o=="number"&&(a[t]=n+(o-n)*s)}return a}function P(i,e){const s=Math.min(Math.max(e,0),i[i.length-1][0]);let a=0;for(;a<i.length-2&&s>=i[a+1][0];)a++;const t=i[a],n=i[a+1],o=Math.min(1,Math.max(0,(s-t[0])/Math.max(.001,n[0]-t[0])));return t[1]+(n[1]-t[1])*o}function he(i){const e=p[p.length-1],s=Math.min(Math.max(i,0),e-.001);let a=0;for(;a<S.length-1&&s>=p[a+1];)a++;const t=p[a],n=p[a+1]??e,o=Math.min(1,Math.max(0,(s-t)/Math.max(.001,n-t))),r=a<S.length-1,h=n-s,m=r?le(1-Math.min(1,h/re)):0,f=S[a],d=r?S[a+1]:f;return{params:ce(f,d,m),actIndex:a,localT:o,blend:m}}const de=[[0,0],[12,.03],[30,.12],[78,.28],[104,.44],[110,.41],[118,.34],[128,.33],[139.7,.33],[140.3,.52],[143.8,.56],[144.4,.66],[150,.72],[162,.86],[167.8,.87],[168.3,.905],[173.7,.91],[174.3,.945],[179.7,.95],[180.3,.975],[183.7,.98],[184.3,1],[200.042,1]];function fe(i){return P(de,i)}const me=[[0,1.45],[12,1.45],[40,1.25],[78,1.15],[104,1.22],[139.7,1.22],[140.3,1.08],[162,1.02],[164,1],[178,.3],[184,.28],[200.042,.28]];function B(i){return P(me,i)}const ue=[[0,.25],[12,.4],[40,.5],[78,.65],[104,.55],[105.5,.3],[108,.35],[126,.4],[139.7,.45],[140.3,.9],[144,1],[148,.95],[161.7,.9],[162.3,.55],[168,.45],[174,.35],[180,.25],[184,.12],[196,.02],[200.042,0]];function pe(i){return P(ue,i)}const ge=(()=>{const i=[0];for(let e=0;e<S.length;e++){const s=p[e+1]-p[e];i.push(i[e]+S[e].lifeRate*s)}return i})();function be(i){const e=p[p.length-1],s=Math.min(Math.max(i,0),e);let a=0;for(;a<S.length-1&&s>=p[a+1];)a++;return ge[a]+S[a].lifeRate*(s-p[a])}class A{slots;constructor(e){this.slots=[];for(let s=0;s<e;s++)this.slots.push(new Q(0,0,0,0))}fire(){let e=this.slots.findIndex(a=>a.w<=0);if(e<0){e=0;let a=this.slots[0].z;for(let t=1;t<this.slots.length;t++)this.slots[t].z>a&&(a=this.slots[t].z,e=t)}const s=this.slots[e];return s.z=0,s.w=1,e}age(e,s){for(let a=0;a<this.slots.length;a++){const t=this.slots[a];t.w<=0||(t.z+=e,s>0&&t.z>=s&&(t.w=0))}}clearAll(){for(let e=0;e<this.slots.length;e++)this.slots[e].w=0}}function M(i,e){if(e<=0)return 1/0;const s=e/60;return-Math.log(1-i())/s}const ve=12,Se=1.5,we=.18,Ee=.035,G=.008,ke=8,ye=8,_e=1.5,Ae=.09,Re=.28,xe=1.02,Te=1+$*1*.5,Le=.3,Oe=8,H=14,K=1.5,Me=10,Ne=2.5,U=.45,Pe=1.2,Ce=30,Ie=.6,Fe=.3,De=4,Be=3,Ge=Fe+De+Be,He=.6,Ke=1.2,Ue=10,R=1.5,W=5e-4,w=.35,We=6,z=.0035,ze=.05,Xe=.012,Ye=.8,X=.006,je=4,Y=140,j=144,qe=[168,174,180],q=184,x=.5,Ze=1,Ve=.7,Qe=.45,$e=.6,Je=1.6,et=.5,tt=1.5,at=.07,st=.15,ot=.5,N=188,T=191,L=194,Z=199;function V(i){const e=Math.min(1,Math.max(0,i));return e*e*(3-2*e)}function it(i){if(i<N)return{phase:0,t:0,seedFade:1};if(i<T){const e=(i-N)/(T-N);return{phase:1,t:V(e),seedFade:1}}if(i<L){const e=(i-T)/(L-T);return{phase:2,t:V(e),seedFade:1}}if(i<Z){const e=(i-L)/(Z-L);return{phase:3,t:e,seedFade:1-e}}return{phase:0,t:0,seedFade:0}}class nt{renderer;scene;camera;quad;material;rand;full=!0;cover=new v(1,1);canvasPxW=1;canvasPxH=1;section=null;forceLifeFast=!1;pinnedBreath=null;pinnedSterile=null;pinnedGlint=null;seedOff=new v;beatBonus=0;bassSlow=0;highFast=0;breathPhase=0;motionPhase=0;soloMode=0;strikePool;strikeB=[];forceStrikeAlways=!1;healOverride=null;forceGhostAlways=!1;bassFast=0;bassOnsetEma=0;onsetCooldown=0;tick=0;strikeTimeToNext=0;lastSongTime=0;rMaxEff=0;bloomPool;bloomCooldown=0;bloomTimeToNext=0;forceBloomAlways=!1;pokePool;ripplePool;pan=new v(0,0);panDisplay=new v(0,0);driftAngle=0;held=!1;dragDx=0;dragDy=0;velX=0;velY=0;shake=0;flash=0;crackle=0;crackleSeed=0;highOnsetEma=0;crackleCooldown=0;crackleTimeToNext=0;forceCrackleAlways=!1;crackleDebugTimer=0;forceSeedPointAlways=!1;crackMult=1;init(e){const{renderer:s,seed:a,quality:t}=e;this.renderer=s,this.rand=oe(a^3009248801),this.full=t.level==="full";const n=new URLSearchParams(location.search),o=n.get("solo");let r=0;if(o==="biomass")r=1;else if(o==="front")r=2;else if(o==="events")r=3;else if(o==="ghosts")r=4;else if(o==="ground")r=6;else{const l=o!==null?parseInt(o,10):NaN;r=Number.isFinite(l)?l:0}this.soloMode=r,this.forceLifeFast=n.get("life")==="fast",this.forceStrikeAlways=n.get("strike")==="always",this.forceBloomAlways=n.get("bloom")==="always",this.forceGhostAlways=n.get("ghost")==="always",this.forceCrackleAlways=n.get("crackle")==="always",this.forceSeedPointAlways=n.get("seedpt")==="always";const h=n.get("heal");h==="off"?this.healOverride=0:h==="on"&&(this.healOverride=.22);const m=n.get("breath");if(m!==null){const l=parseFloat(m);Number.isNaN(l)||(this.pinnedBreath=Math.min(1,Math.max(0,l)))}const f=n.get("sterile");if(f!==null){const l=parseFloat(f);Number.isNaN(l)||(this.pinnedSterile=Math.min(1,Math.max(0,l)))}const d=n.get("glint");if(d!==null){const l=parseFloat(d);Number.isNaN(l)||(this.pinnedGlint=Math.min(1,Math.max(0,l)))}const u=n.get("crack");if(u!==null){const l=parseFloat(u);Number.isNaN(l)||(this.crackMult=Math.min(6,Math.max(.25,l)))}const g=()=>(this.rand()<.5?-1:1)*(.1+this.rand()*.14),b=new v(.5+g(),.5+g()),E=this.rand()*.55,_=this.rand()*.35,c=this.rand()<.5?new v(1+E,1-_):new v(1-_,1+E);this.seedOff.set(this.rand()*10,this.rand()*10),this.driftAngle=this.rand()*Math.PI*2;const k=new v(.5+(this.rand()-.5)*.12,.5+(this.rand()-.5)*.12),y=this.full?8:6;this.strikePool=new A(y);for(let l=0;l<y;l++)this.strikeB.push(new Q(0,0,0,0));const C=3,I=4,F=4;this.bloomPool=new A(C),this.pokePool=new A(I),this.ripplePool=new A(F),this.scene=new J,this.camera=new ee(-1,1,1,-1,0,1),this.material=new te({vertexShader:ie,fragmentShader:ne({strikeSlots:y,bloomSlots:C,pokeSlots:I,rippleSlots:F,lobes:this.full?5:3,fbmOct:this.full?3:2,grain:this.full,fissures:this.full?4:3}),depthTest:!1,depthWrite:!1,uniforms:{uCover:{value:new v(1,1)},uZoom:{value:1},uFieldPxScale:{value:0},uPan:{value:this.panDisplay},uTime:{value:0},uSterile:{value:0},uSoloMode:{value:r},uLifeClock:{value:0},uPresence:{value:0},uBreath:{value:0},uBreathPhase:{value:0},uMotionPhase:{value:0},uGroundPhase:{value:0},uHeat:{value:0},uPaleness:{value:0},uPocket:{value:b},uStretch:{value:c},uFrontDrift:{value:new v(0,0)},uFrontNoise:{value:0},uRMax:{value:0},uScrubGlow:{value:0},uGlint:{value:0},uWatermark:{value:0},uSterileSpec:{value:0},uTick:{value:0},uStrikeA:{value:this.strikePool.slots},uStrikeB:{value:this.strikeB},uBloom:{value:this.bloomPool.slots},uBloomAmp:{value:0},uPoke:{value:this.pokePool.slots},uRipple:{value:this.ripplePool.slots},uGhostAmp:{value:0},uGhostForce:{value:this.forceGhostAlways?1:0},uEnergy:{value:0},uGrade:{value:0},uVignette:{value:0},uFlash:{value:0},uCrackle:{value:0},uCrackleSeed:{value:0},uFinalPhase:{value:0},uFinalT:{value:0},uSeedFade:{value:0},uFinalPos:{value:k}}}),this.quad=new ae(new se(2,2),this.material),this.scene.add(this.quad);const D=s.domElement;this.resize(D.clientWidth||1,D.clientHeight||1)}update(e,s){const a=s.time,t=this.material.uniforms;t.uTime.value+=e,t.uSterile.value=this.soloMode===3?0:this.pinnedSterile!==null?this.pinnedSterile:fe(a),t.uZoom.value=B(a);const n=he(a),o=n.actIndex===3||n.actIndex===4?S[n.actIndex]:n.params;this.section=n;const r=pe(a);t.uEnergy.value=r,t.uGrade.value=o.grade,t.uVignette.value=o.vignette,Math.abs(a-this.lastSongTime)>Me&&(this.strikePool.clearAll(),this.strikeTimeToNext=0,this.bloomPool.clearAll(),this.bloomTimeToNext=0,this.pokePool.clearAll(),this.ripplePool.clearAll(),this.crackleTimeToNext=0),this.lastSongTime<Y&&a>=Y&&a-this.lastSongTime<x&&(this.flash=Math.max(this.flash,Ze)),this.lastSongTime<j&&a>=j&&a-this.lastSongTime<x&&(this.fireStrike(o,Je,et,!0),this.flash=Math.max(this.flash,Ve));for(const c of qe)this.lastSongTime<c&&a>=c&&a-this.lastSongTime<x&&(this.flash=Math.max(this.flash,Qe));this.lastSongTime<q&&a>=q&&a-this.lastSongTime<x&&(this.flash=Math.max(this.flash,$e)),this.lastSongTime=a;const h=this.forceSeedPointAlways?{phase:3,t:0,seedFade:1}:it(a);t.uFinalPhase.value=h.phase,t.uFinalT.value=h.t,t.uSeedFade.value=h.seedFade;const m=this.forceLifeFast?ve:1;t.uLifeClock.value=be(a)*m+this.beatBonus,t.uPresence.value=o.clumpPresence;const f=this.bassSlow;this.bassSlow+=(s.bass-this.bassSlow)*Math.min(1,e*Se),t.uBreath.value=this.pinnedBreath!==null?this.pinnedBreath:this.bassSlow*o.breathAmp,t.uZoom.value=t.uZoom.value*(1-Xe*t.uBreath.value);{const c=t.uZoom.value,k=Math.max(this.cover.x,this.cover.y),y=Math.max(1,Math.min(this.canvasPxW,this.canvasPxH));t.uFieldPxScale.value=k/(Math.max(1e-4,c)*y)}this.breathPhase+=e*we,t.uBreathPhase.value=this.breathPhase,this.motionPhase+=e*o.motionSpeed*(.5+s.mid),t.uMotionPhase.value=this.motionPhase,t.uGroundPhase.value=a*Ee,t.uHeat.value=o.heat,t.uPaleness.value=o.paleness,t.uGhostAmp.value=this.forceGhostAlways?1:o.ghostAmp;const d=t.uFrontDrift.value;if(d.x+=e*G,d.y+=e*G*.83,t.uFrontNoise.value=o.frontNoise,t.uScrubGlow.value=o.scrubGlow*(.6+.8*this.bassSlow),this.highFast+=(s.high-this.highFast)*Math.min(1,e*ke),t.uGlint.value=this.pinnedGlint!==null?this.pinnedGlint:o.glintAmp*this.highFast,this.highOnsetEma+=(this.highFast-this.highOnsetEma)*Math.min(1,e*tt),this.crackleCooldown=Math.max(0,this.crackleCooldown-e),this.crackleCooldown<=0&&this.highFast-this.highOnsetEma>at&&(this.crackleCooldown=st,this.fireCrackle()),this.forceCrackleAlways)this.crackleDebugTimer-=e,this.crackleDebugTimer<=0&&(this.fireCrackle(),this.crackleDebugTimer=ot);else{const c=o.crackleRate*(.5+r);if(c>0)for(this.crackleTimeToNext-=e;this.crackleTimeToNext<=0;)this.fireCrackle(),this.crackleTimeToNext+=M(this.rand,c)}this.crackle*=Math.exp(-7*e),t.uCrackle.value=this.crackle,t.uCrackleSeed.value=this.crackleSeed,t.uWatermark.value=o.watermark,t.uSterileSpec.value=o.sterileSpec,this.bassFast+=(s.bass-this.bassFast)*Math.min(1,e*ye),this.bassOnsetEma+=(this.bassFast-this.bassOnsetEma)*Math.min(1,e*_e),this.onsetCooldown=Math.max(0,this.onsetCooldown-e),this.onsetCooldown<=0&&this.bassFast-this.bassOnsetEma>Ae&&(this.onsetCooldown=Re,this.tick=1,this.fireStrike(o)),this.tick*=Math.exp(-7*e),t.uTick.value=this.tick,this.flash*=Math.exp(-3.4*e),t.uFlash.value=this.flash;const u=this.forceStrikeAlways?60:o.strikeRate;if(u>0)for(this.strikeTimeToNext-=e;this.strikeTimeToNext<=0;)this.fireStrike(o),this.strikeTimeToNext+=M(this.rand,u);this.ageStrikes(e),this.bloomCooldown=Math.max(0,this.bloomCooldown-e),this.bloomCooldown<=0&&f<U&&this.bassSlow>=U&&(this.bloomCooldown=Pe,this.fireBloom());const g=this.forceBloomAlways?Ce:o.bloomRate;if(g>0)for(this.bloomTimeToNext-=e;this.bloomTimeToNext<=0;)this.fireBloom(),this.bloomTimeToNext+=M(this.rand,g);t.uBloomAmp.value=o.bloomAmp,this.bloomPool.age(e,Ne),this.pokePool.age(e,Ge),this.ripplePool.age(e,Ke);const b=t.uZoom.value;if(this.held){if(e>1e-5){const c=Math.min(1,e*Ue),k=Math.min(R,Math.max(-R,this.dragDx/e)),y=Math.min(R,Math.max(-R,this.dragDy/e));this.velX+=(k-this.velX)*c,this.velY+=(y-this.velY)*c}this.dragDx=0,this.dragDy=0}else if(this.velX!==0||this.velY!==0){this.pan.x+=this.velX*this.cover.x/b*e,this.pan.y+=this.velY*this.cover.y/b*e;const c=Math.exp(-2.5*e);this.velX*=c,this.velY*=c,Math.abs(this.velX)<W&&(this.velX=0),Math.abs(this.velY)<W&&(this.velY=0)}else{this.driftAngle+=e*ze;const c=Math.cos(this.driftAngle),k=Math.sin(this.driftAngle);this.pan.x+=c*o.driftSpeed*z*e,this.pan.y+=k*o.driftSpeed*z*e}this.pan.x>w?(this.pan.x=w,this.velX=0):this.pan.x<-w&&(this.pan.x=-w,this.velX=0),this.pan.y>w?(this.pan.y=w,this.velY=0):this.pan.y<-w&&(this.pan.y=-w,this.velY=0),this.shake*=Math.exp(-6*e);const E=t.uTime.value,_=Math.sin(E*41)*this.shake*X,O=Math.sin(E*53+1.3)*this.shake*X;this.panDisplay.set(this.pan.x+_,this.pan.y+O)}pickLivingPoint(e=1){const s=this.material.uniforms,a=s.uCover.value,t=s.uZoom.value,n=this.pan,o=s.uPocket.value,r=s.uStretch.value,h=s.uSterile.value,m=this.rMaxEff*(1-h);for(let f=0;f<Oe;f++){let d=this.rand(),u=this.rand();d=.5+(d-.5)*e,u=.5+(u-.5)*e;const g=(d-.5)*a.x/t+.5+n.x,b=(u-.5)*a.y/t+.5+n.y,E=(g-o.x)*r.x,_=(b-o.y)*r.y;if(Math.hypot(E,_)-m<-.06)return{x:g,y:b}}return null}fireStrike(e,s=1,a=1,t=!1){const n=this.pickLivingPoint(a);if(!n)return;const o=this.strikePool.fire(),r=this.strikePool.slots[o];r.x=n.x,r.y=n.y,s*=this.crackMult;const h=e.strikeSize*s*(.8+this.rand()*.5),m=1.15+this.rand()*.5,f=this.rand()*Math.PI*2,d=t?0:this.healOverride!==null?this.healOverride:e.strikeHeal;this.strikeB[o].set(h,m,f,d),this.section&&this.section.actIndex===je&&(this.shake=e.shake*Ye)}fireBloom(){const e=this.pickLivingPoint(Ie);if(!e)return;const s=this.bloomPool.fire(),a=this.bloomPool.slots[s];a.x=e.x,a.y=e.y}fireBloomAt(e,s){const a=this.bloomPool.fire(),t=this.bloomPool.slots[a];t.x=e,t.y=s}firePoke(e,s){const a=this.pokePool.fire(),t=this.pokePool.slots[a];t.x=e,t.y=s}fireRipple(e,s){const a=this.ripplePool.fire(),t=this.ripplePool.slots[a];t.x=e,t.y=s}fireCrackle(){this.crackle=1,this.crackleSeed++}ageStrikes(e){this.strikePool.age(e,0);const s=this.strikePool.slots;for(let a=0;a<s.length;a++){const t=s[a];if(t.w<=0)continue;const n=this.strikeB[a].w,o=t.z;if(n>0){const r=Le+1/n;o>=r&&(t.w=0)}else{const r=H-K;o>=H?t.w=0:o>=r&&(t.w=Math.max(0,1-(o-r)/K))}}}resize(e,s){if(!this.material||e<=0||s<=0)return;const a=Math.min(3.5,Math.max(.28,e/s));a>=1?this.cover.set(a,1):this.cover.set(1,1/a),this.material.uniforms.uCover.value.copy(this.cover),this.canvasPxW=this.renderer.domElement.width||1,this.canvasPxH=this.renderer.domElement.height||1,this.rMaxEff=this.computeRMaxEff(),this.material.uniforms.uRMax.value=this.rMaxEff}computeRMaxEff(){const e=this.material.uniforms,s=e.uPocket.value,a=e.uStretch.value,t=B(0),n=.5*this.cover.x/t,o=.5*this.cover.y/t;let r=0;for(const h of[-1,1])for(const m of[-1,1]){const f=.5+h*n,d=.5+m*o,u=(f-s.x)*a.x,g=(d-s.y)*a.y,b=Math.hypot(u,g);b>r&&(r=b)}return r*Te*xe}pointer(e){const s=this.material.uniforms.uZoom.value,a=this.cover;if(e.type==="down"){this.held=!0,this.dragDx=0,this.dragDy=0,this.velX=0,this.velY=0;const t=(e.x-.5)*a.x/s+.5+this.pan.x,n=(e.y-.5)*a.y/s+.5+this.pan.y;(this.section?this.section.actIndex:0)===We?this.fireBloomAt(t,n):(this.firePoke(t,n),this.tick=Math.max(this.tick,He)),this.fireRipple(t,n);return}if(e.type==="move"){if(!this.held)return;this.pan.x+=e.dx*a.x/s,this.pan.y+=e.dy*a.y/s,this.dragDx+=e.dx,this.dragDy+=e.dy;return}if(e.type==="up"){this.held=!1;return}this.held=!1,this.velX=0,this.velY=0,this.dragDx=0,this.dragDy=0}render(){this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera)}dispose(){this.material.dispose(),this.quad.geometry.dispose(),this.renderer.setRenderTarget(null)}}const rt={default:()=>new nt},ht=rt.default;export{ht as default};
//# sourceMappingURL=index-C-vxB7T7.js.map
